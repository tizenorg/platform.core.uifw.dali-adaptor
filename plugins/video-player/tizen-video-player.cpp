...
void TizenVideoPlayer::SetRenderingTarget( Any target )
{
  ...
  else (target.GetType() == typeid( Dali::Window ) )
  {
    int width, height;
    Dali::Window window = AnyCast< Dali::Window >( target );
    mEcoreWlWindow = AnyCast< Ecore_Wl_Window* >( window.GetNativeHandle() );
