...
#include <wayland/ecore-wl-render-surface.h>
...
void TizenVideoPlayer::SetRenderingTarget( Any target )
{
  ...
  else if( target.GetType() == typeid( RenderSurface* ) )
  {
    Dali::RenderSurface* renderSurface = AnyCast< Dali::RenderSurface* >( target );
    Dali::ECore::EcoreWlRenderSurface* ecoreRenderSurface = dynamic_cast< Dali::ECore::EcoreWlRenderSurface* >( renderSurface );
    ...
    error = player_set_ecore_wl_display( mPlayer, PLAYER_DISPLAY_TYPE_OVERLAY, ecoreRenderSurface->GetWlWindow(), 0, 0, size.x, size.y );
    LogPlayerError( error );
  }
  else
  {
...
Dali::VideoPlayerPlugin::DisplayRotation TizenVideoPlayer::GetDisplayRotation()
{
  if( mNativeImageSourcePtr != NULL )
  {
    DALI_LOG_ERROR( "GetDisplayRotation is for window rendering target.\n" );
