...
namespace
{
  ...
  {
  public:
    FrameworkNotificationJob(int (*function_ptr) (void *), void* context, bool running) : mFunctionPtr(function_ptr),
                                                                                          mContext(context)
    {
    }
...
void androidcore_pause()
{
  ...
  if (isAppActive)
  {
    Jobs::JobHandle pauseJob = new FrameworkNotificationJob(currentOps->pause, currentOps->data, false);
...
void androidcore_trigger_resume()
{
  ...
  {
    while (currentOps == NULL)
    {
      usleep(100);
    }

    Jobs::JobHandle surfaceCreatedJob = new FrameworkNotificationJob(currentOps->surfaceCreated, currentOps->data, true);
    Jobs::add_job(Jobs::get_main_queue(), surfaceCreatedJob);
    isDaliSurfaceCreated = true;
  }

  Jobs::JobHandle resumeJob = new FrameworkNotificationJob(currentOps->resume, currentOps->data, true);
...
void androidcore_surfaceDestroyed(ANativeWindow* window)
{
  ...
  {
    Jobs::JobHandle surfaceDestroyedJob = new FrameworkNotificationJob(currentOps->surfaceDestroyed, currentOps->data, false);
