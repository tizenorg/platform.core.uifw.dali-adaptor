...
static SlpFace* LoadFontFile(const std::string &fileName, FT_Library freeType)
{
  ...
  if (androidcore_LoadAsset(fileName, data))
  {
    if( data.Size() > 0u )
    {
      ...
      slpFace = new SlpFace;
      slpFace->buffer = new FT_Byte[data.Size()];

      // read the file
      memcpy(slpFace->buffer, data.Begin(), data.Size());

      // create a freetype face from the memory buffer
      FT_Error retVal = FT_New_Memory_Face(freeType, slpFace->buffer, data.Size(), 0, &slpFace->face);
      if (retVal != FT_Err_Ok)
      {
        LOGW("FT_New_Face failed on %s\n", fileName.c_str());
        delete slpFace;
        slpFace = NULL;
      }
    }
                      ...
                      FT_Library freeType)
{
  ...
  if (NULL != slpFace)
  {
    ...
    if (retVal != FT_Err_Ok)
    {
      LOGW("FT_Set_Char_Size failed on %s\n", fileName.c_str());
      delete slpFace;
      slpFace = NULL;
    }
    else
    {
      LOGI ("Loading font %s Glyphs(%d) Points(%f) DPI(%d %d) Units/EM(%d) PPEM(%d %d) Scale(%f %f)\n",
                     fileName.c_str(), face->num_glyphs,
                     roundf(pointSize.value), dpiHor, (int)dpiVer, (int)face->units_per_EM,
                      ...
                      FT_Library freeType)
{
  ...
  if (NULL != slpFace)
  {
    ...
    if (retVal != FT_Err_Ok)
    {
      LOGW("FT_Set_Pixel_Sizes failed on %s\n", fileName.c_str());
      delete slpFace;
      slpFace = NULL;
    }
    else
    {
      LOGI ( "Loading font %s Glyphs(%d) Pixels(%d) Units/EM(%d) PPEM(%d %d) Scale(%f %f)\n",
                     fileName.c_str(), face->num_glyphs,
                                  ...
                                  const bool renderBitmap, const bool highQuality)
{
  ...
  if (renderBitmap)
  {
    ...
    if (glyph->format != FT_GLYPH_FORMAT_BITMAP)
    {
      ...
      if(retVal != FT_Err_Ok)
      {
        LOGW("FT_Glyph_To_Bitmap failed %d\n", retVal);
        FT_Done_Glyph(glyph);
        return NULL;
      }
    }
    ...
    LOGI("%s(%c %f %f %f %f %f %f) [%.2f %.2f %.2f %.2f %f %f]\n",
