#ifndef __DALI_ANDROID_NATIVE_IMAGE_H__
#define __DALI_ANDROID_NATIVE_IMAGE_H__
...
class AndroidNativeImage;
typedef Dali::IntrusivePtr<AndroidNativeImage> AndroidNativeImagePtr;

/**
 * Used for displaying/streaming Android SurfaceTextures.
 *
 */
class AndroidNativeImage : public NativeImage
{
public:
  ...
  /**
   * Create a new AndroidNativeImage.
   ...
   */
  static AndroidNativeImagePtr New(unsigned int width, unsigned int height, ColorDepth depth, Application application);
   ...
   * @return A smart-pointer to a newly allocated image.
   */
  static AndroidNativeImagePtr New( unsigned int width, unsigned int height, ColorDepth depth,  Adaptor& adaptor );

  /**
   * For GraphicBuffer
   * @param[in] image the image to be uploaded to the graphicbuffer 
   */
...
private:
   ...
   */
  AndroidNativeImage(unsigned int width, unsigned int height, ColorDepth depth, Application application);
   ...
   * @param[in] pixmap contains either: pixmap of type X11 Pixmap , a Ecore_X_Pixmap or is empty
   */
  AndroidNativeImage(unsigned int width, unsigned int height, ColorDepth depth, Adaptor& adaptor);
...
public:
   ...
   */
  AndroidNativeImage& operator=(const AndroidNativeImage& rhs);
...
private:
  ...
  Internal::Adaptor::AndroidNativeImage* mImpl;
