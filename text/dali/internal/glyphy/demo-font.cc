                 ...
                 GlyphInfo        *glyphInfo )
{
  ...
  if (FT_Err_Ok != glyphy_freetype_outline_decompose(&face->glyph->outline, font->acc))
  {
  ...
  *advance = face->glyph->metrics.horiAdvance / (double) upem;
  ...
  {
    glyphInfo->width  = (extents->max_x - extents->min_x);
    glyphInfo->height = (extents->max_y - extents->min_y);

    glyphInfo->xBearing = extents->min_x;
