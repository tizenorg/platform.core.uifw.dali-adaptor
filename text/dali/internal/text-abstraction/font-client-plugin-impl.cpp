                                           ...
                                           int desiredFixedSize )
{
  ...
  for( unsigned int i=0; i<size; ++i )
  {
        ...
        fontId-1 < mFontCache.size() )
    {
      ...
      if( ! font.mVectorData )
      {
        font.mVectorData = demo_font_create( font.mFreeTypeFace );
      }

      demo_font_lookup_glyph( font.mVectorData, array[i] );
...
void FontClient::Plugin::CreateVectorBlob( FontId fontId, GlyphIndex glyphIndex, VectorBlob*& blob, unsigned int& blobLength, unsigned int& nominalWidth, unsigned int& nominalHeight )
{
      ...
      fontId-1 < mFontCache.size() )
  {
    ...
    demo_font_lookup_blob( font.mVectorData, fontId, glyphIndex, &blob, &blobLength, &nominalWidth, &nominalHeight );
  }
  else
  {
    blob = NULL;
