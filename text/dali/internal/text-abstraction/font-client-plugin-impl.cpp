                                          ...
                                          bool horizontal )
{
  ...
  for( unsigned int i=0; i<size; ++i )
  {
        ...
        fontId-1 < mFontCache.size() )
    {
      ...
      if ( !mFontCache[fontId-1].mPointSize )
      {
                                       ...
                                       bool cacheDescription )
{
  ...
  if( FT_Err_Ok == error )
  {
    ...
    if ( ftFace->num_fixed_sizes && ftFace->available_sizes )
    {
      ...
      if ( FT_Err_Ok == error )
      {
        // Zero Pointsize will indicate that the font is a fixed sized bitmap, pass width in the descender for metrics
        ...
        mFontCache.push_back( CacheItem( ftFace, path, 0, faceIndex, metrics ) );
        id = mFontCache.size();
