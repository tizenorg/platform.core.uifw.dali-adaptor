...
void FontClient::Plugin::CreateGlyphyBlob( FontId fontId, GlyphIndex glyphIndex, GlyphyBlob*& blob, unsigned int& blobLength )
{
  if( fontId > 0 &&
      fontId-1 < mFontCache.size() )
  {
    ...
    demo_font_lookup_blob( font.mGlyphyData, fontId, glyphIndex, &blob, &blobLength );
  }
  else
  {
    blob = NULL;
