        ...
        fontId-1 < mFontCache.size() )
    {
      ...
      if ( mFontCache[fontId-1].mIsFixedSizeBitmap )
      {
        ...
        if ( FT_Err_Ok == error )
        {
          ...
          array[i].yBearing = mFontCache[ fontId -1 ].mFixedHeightPixels - GetBitmapBase( fontId, array[i].index );
        }
        else
        {
...
float FontClient::Plugin::GetBitmapBase( FontId fontId, GlyphIndex glyphIndex )
{
  // Search bitmap base cache to see if there is an entry for this fontId
  std::vector< BitmapBaseCacheEntry >::iterator bbcIt = mBitmapBaseCache.begin();
  for ( ; bbcIt != mBitmapBaseCache.end(); ++bbcIt )
  {
    if ( bbcIt->mFontId == fontId )
    {
      // Check for an entry for this glyph
      Vector< BitmapBaseGlyphEntry >::Iterator bbgIt = bbcIt->mBaseList.Begin();
      for ( ; bbgIt != bbcIt->mBaseList.End(); ++ bbgIt )
      {
  ...
  if( FT_Err_Ok ==  FT_Load_Glyph( ftFace, glyphIndex, FT_LOAD_COLOR ) )
  {
    ...
    bbcIt = mBitmapBaseCache.begin();
    for ( ; bbcIt != mBitmapBaseCache.end(); ++bbcIt )
    {
      if ( bbcIt->mFontId == fontId )
      {
        bbcIt->mBaseList.PushBack( entry );
        return entry.mBase;
      }
    }
    // No record for this font, so create a new one
    BitmapBaseCacheEntry cacheEntry;
    cacheEntry.mFontId = fontId;
    cacheEntry.mBaseList.PushBack( entry );
    mBitmapBaseCache.push_back( cacheEntry );
  }
  return entry.mBase;
}
