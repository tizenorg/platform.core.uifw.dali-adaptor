                                          ...
                                          int maxFixedSize )
{
  ...
  {
    return GetVectorMetrics( array, size, horizontal, maxFixedSize );
  }

  return GetBitmapMetrics( array, size, horizontal, maxFixedSize );
}
  ...
  for( unsigned int i=0; i<size; ++i )
  {
        ...
        fontId-1 < mFontCache.size() )
    {
      ...
      if( ! font.mGlyphyData )
      {
        ...
        font.mGlyphyData = demo_font_create( font.mFreeTypeFace, scale );
      }
...
void FontClient::Plugin::CreateGlyphyBlob( FontId fontId, GlyphIndex glyphIndex, GlyphyBlob*& blob, unsigned int& blobLength, unsigned int& nominalWidth, unsigned int& nominalHeight )
{
  if( fontId > 0 &&
      fontId-1 < mFontCache.size() )
  {
    ...
    demo_font_lookup_blob( font.mGlyphyData, fontId, glyphIndex, &blob, &blobLength, &nominalWidth, &nominalHeight );
  }
  else
  {
    blob = NULL;
