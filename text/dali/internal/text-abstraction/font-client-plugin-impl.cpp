...
float FontClient::Plugin::GetGlyphDescenderForBitmap( FontId fontId, GlyphIndex glyphIndex )
{
  ...
  std::vector< GlyphDescenderCacheEntry >::iterator gDcIt = mGlyphDescenderCache.begin();
  for ( ; gDcIt != mGlyphDescenderCache.end(); ++gDcIt )
  {
    if ( gDcIt->mFontId == fontId )
    {
      // Check for an entry for this glyph
      Vector< GlyphDescenderEntry >::Iterator gDeIt = gDcIt->mGlyphDescenderList.Begin();
      for ( ; gDeIt != gDcIt->mGlyphDescenderList.End(); ++ gDeIt )
      {
        if ( gDeIt->mGlyphIndex == glyphIndex )
        {
          return ( gDeIt->mGlyphDescender );
        }
      }
    }
  }
  ...
  if( FT_Err_Ok ==  FT_Load_Glyph( ftFace, glyphIndex, FT_LOAD_COLOR ) )
  {
    ...
    gDcIt = mGlyphDescenderCache.begin();
    for ( ; gDcIt != mGlyphDescenderCache.end(); ++gDcIt )
    {
      if ( gDcIt->mFontId == fontId )
      {
        gDcIt->mGlyphDescenderList.PushBack( entry );
