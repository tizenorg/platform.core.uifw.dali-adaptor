...
struct FontClient::Plugin
{
   ...
   */
  bool GetBitmapMetrics( GlyphInfo* array, uint32_t size, bool horizontal, int maxFixedSize );

  /**
   * Helper for GetGlyphMetrics when using vectors
   */
  bool GetVectorMetrics( GlyphInfo* array, uint32_t size, bool horizontal, int maxFixedSize );
