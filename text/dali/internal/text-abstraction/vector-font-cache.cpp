  ...
  if( mImpl )
  {
        ...
        vectorFontId-1 < mImpl->mVectorFonts.size() )
    {
      ...
      if( foundGlyph )
      {
        ...
        // preserve glyphInfo.fontId
        glyphInfo.width       = glyph->glyphInfo.width;
        glyphInfo.height      = glyph->glyphInfo.height;
        glyphInfo.xBearing    = glyph->glyphInfo.xBearing;
        glyphInfo.yBearing    = glyph->glyphInfo.yBearing;
        glyphInfo.advance     = glyph->glyphInfo.advance;
        glyphInfo.scaleFactor = glyph->glyphInfo.scaleFactor;
      }
      else
      {
