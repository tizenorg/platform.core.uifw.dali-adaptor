...
void VectorFontCache::GetGlyphMetrics( FontId fontId, GlyphInfo& glyphInfo )
{
  if( mImpl )
  {
        ...
        fontId-1 < mImpl->mVectorFonts.size() )
    {
      ...
      {
        if( cache[ foundIndex ].glyphInfo.index == glyphInfo.index )
        {
          foundIndex = i;
          foundGlyph = true;
          break;
      ...
      if( foundGlyph )
      {
        glyphInfo = cache[ foundIndex ].glyphInfo;
      }
      else
      {
        VectorGlyph* newGlyph = VectorGlyph::New( font->mFace, glyphInfo.index, mAccumulator );
...
void VectorFontCache::GetVectorBlob( FontId fontId, GlyphIndex glyphIndex, VectorBlob*& blob, unsigned int& blobLength, unsigned int& nominalWidth, unsigned int& nominalHeight )
{
  if( mImpl )
  {
    if( fontId > 0 &&
        fontId-1 < mImpl->mVectorFonts.size() )
    {
      //GlyphCache& cache = mImpl->mVectorFonts[ fontId-1 ].mGlyphCache;
