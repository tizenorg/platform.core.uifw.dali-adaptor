...
void VectorFontCache::GetGlyphMetrics( FontId fontId, GlyphInfo& glyphInfo )
{
  if( mImpl )
  {
    if( fontId > 0 &&
        fontId-1 < mImpl->mVectorFonts.size() )
    {
      VectorFont* font = mImpl->mVectorFonts[ fontId-1 ];
      ...
      {
        VectorGlyph* newGlyph = VectorGlyph::New( font->mFace, glyphInfo.index, mImpl->mAccumulator );
...
void VectorFontCache::GetVectorBlob( FontId fontId, GlyphIndex glyphIndex, VectorBlob*& blob, unsigned int& blobLength, unsigned int& nominalWidth, unsigned int& nominalHeight )
{
  if( mImpl )
  {
    if( fontId > 0 &&
        fontId-1 < mImpl->mVectorFonts.size() )
    {
      ...
      {
        VectorGlyph* newGlyph = VectorGlyph::New( font->mFace, glyphIndex, mImpl->mAccumulator );
...
bool VectorFontCache::FindFont( const string& url, FontId& fontId ) const
{
  fontId = 0u;
