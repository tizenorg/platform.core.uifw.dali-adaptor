...
BaseHandle Create()
{
  ...
  if ( service )
  {
    Dali::TextAbstraction::TextAbstractionInterface textInterface = Dali::TextAbstraction::TextAbstractionInterface( new TextAbstractionInterface);
    service.Register( typeid( textInterface ), textInterface );
    BaseHandle handle(textInterface);
    return handle;
  }
  return BaseHandle();
}
TypeRegistration TEXT_ABSTRACTION_INTERFACE_TYPE( typeid(Dali::TextAbstraction::TextAbstractionInterface), typeid(Dali::BaseHandle), Create, true /* Create Instance At Startup */ );
