...
/**
 * Handles wayland text input interface which handles the input panel (virtual keyboard ).
 *
 */
...
public:
  ...
  /**
   * @brief see if the input panel is visilbe
   * @return panel status
   */
...
public:   ///< Input Panel Signals ( DALi currently only uses signals from IMF interface).
...
public:
  ...
  struct SeatInfo
  {
    ...
    InputMethod::ActionButton mReturnKeyType;
    bool mVisible:1;
    bool mFocused:1;
  };
