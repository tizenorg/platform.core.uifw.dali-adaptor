...
void InputPanelState(void* data,
          WlTextInput* textInput,
          uint32_t state)
{
  TextInputInterface* input = static_cast< TextInputInterface* >( data );
  ...
  printf(" Input Panel State state==%d \n", state);
}

void PreeditString(void* data,
           WlTextInput* textInput,
           uint32_t serial,
           const char* text,
           const char* commit)
{
  ...
  printf("pre-edit string = %s \n",text);

  input->PreeditString( seat, serial, text, commit );
}

void PreeditStyling(void* data,
      WlTextInput* textInput,
      uint32_t index,
      uint32_t length,
      uint32_t style)
{
  ...
  printf("pre-edit style \n");

  input->PreeditString( seat, index, length, style );
           ...
           uint32_t height)
{
  ...
  printf("InputPanelGeometry %d, %d, %d, %d  \n", x,y,width,height);
