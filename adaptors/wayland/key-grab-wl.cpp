...
{
  wl_keyrouter_mode xGrabMode;
  if( grabMode == TOP_POSITION )
  {
    xGrabMode = WL_KEYROUTER_MODE_TOPMOST;
  }
  else if( grabMode == SHARED )
  {
    xGrabMode = WL_KEYROUTER_MODE_SHARED;
  }
  else
  {
    return false;
  ...
  if( keyrouter )
  {
    ...
    int ret = keygrab_sync_request(keyrouter, surface, daliKey, xGrabMode);
    return ret==0;
  }
  else
  {
...
bool UngrabKey( Window window, Dali::KEY daliKey )
{
  ...
  if( keyrouter )
  {
    int ret = keyungrab_sync_request(keyrouter, test_surface, daliKey);
    return ret==0;
  }
  else
  {
