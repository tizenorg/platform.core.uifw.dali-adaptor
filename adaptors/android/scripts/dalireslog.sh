...
function redraw
{
  ...
  tput setab 4; printf %"$TERM_W"s; printf '\n'; tput sgr0

  # print filenames
  local MAXFILENO=$(($TERM_H-$SEPARATOR_H-2)) #leave space for keyboard shortcuts and separator itself
  local count=0
  local index=0
...
#     printPath "${FILENAMES_G[$count]}"
      printLine "${FILENAMES_G[$count]} ${SIZES_G[$count]} ${SIZE_DETAILS_G[$count]}"
    else
      tput sgr0
