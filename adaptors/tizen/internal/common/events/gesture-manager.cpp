...
GestureManager::GestureManager(Vector2 screenSize,CallbackManager* callbackManager)
: mCore( NULL ),
...
void GestureManager::SetCore(Integration::Core& core)
{
  DALI_LOG_INFO( gLogFilter, Debug::Verbose, "SetCore\n" );
...
void GestureManager::Register(const Integration::GestureRequest& request)
{
  ...
  if (mRunning)
  {
    ...
    switch (request.type)
    {
      case Gesture::LongPress:
      {
        GestureDetectorPtr gesture = new LongPressGestureDetector(*mCore, mScreenSize, static_cast<const Integration::LongPressGestureRequest&>(request));
      ...
      case Gesture::Pan:
      {
        GestureDetectorPtr gesture = new PanGestureDetector(*mCore, mScreenSize, static_cast<const Integration::PanGestureRequest&>(request));
        mGestureDetectors.push_back(gesture);
        break;
      }
      ...
      case Gesture::Pinch:
      {
        GestureDetectorPtr gesture = new PinchGestureDetector(*mCore, mScreenSize);
        mGestureDetectors.push_back(gesture);
        break;
      }
      ...
      case Gesture::Tap:
      {
        GestureDetectorPtr gesture = new TapGestureDetector(*mCore, mScreenSize, static_cast<const Integration::TapGestureRequest&>(request));
        mGestureDetectors.push_back(gesture);
        break;
      }
