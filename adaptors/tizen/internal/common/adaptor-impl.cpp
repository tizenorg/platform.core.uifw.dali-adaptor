...
void Adaptor::Initialize()
{
  ...
  // get performance logging options
  unsigned int fpsTrackingSeconds = GetIntegerEnvironmentVariable( DALI_ENV_FPS_TRACKING, 0 ); ;
  unsigned int updateStatusInterval = GetIntegerEnvironmentVariable( DALI_ENV_UPDATE_STATUS_INTERVAL, 0 );
  unsigned int perfLogging = GetIntegerEnvironmentVariable( DALI_ENV_LOG_PERFORMANCE, 0 );
  ...
  if (logOpts & Debug::LogEventThread)
  {
    // install it on this thread (main actor thread)
  ...
  mGestureManager = new GestureManager(Vector2(size.width, size.height), mCallbackManager);
  ...
  mGestureManager->SetCore( *mCore );

  mVSyncMonitor = new VSyncMonitor;

  mUpdateRenderController = new UpdateRenderController( *mCore,
                                                        logFunction, logOpts,
                                                        fpsTrackingSeconds, updateStatusInterval,perfLogging,
                                                        *mNotificationTrigger,
                                                        *mGLES, mEglFactory, mSurface,
...
void Adaptor::Start()
{
  ...
  mEventHandler = new EventHandler( mSurface, *mCore, *mGestureManager, *this, mDragAndDropDetector );
...
void Adaptor::AddObserver( Observer& observer )
{
...
void Adaptor::RemoveObserver( Observer& observer )
{
  ObserverContainer::iterator match ( find(mObservers.begin(), mObservers.end(), &observer) );
