  ...
  stageWidth(0), stageHeight(0)
{
  if ( *argc > 1 )
  {
    ...
    std::vector< int > unhandledOptions;
    ...
    do
    {
      ...
      switch ( shortOption )
      {
        case 0:
        {
          // Check if we want help when not handling
          if ( help )
          {
    ...
    if ( optionProcessed )
    {
      if ( !unhandledOptions.empty() )
      {
        ...
        for ( std::vector< int >::iterator iter = unhandledOptions.begin(), endIter = unhandledOptions.end(); iter != endIter; ++iter )
        {
          (*argv)[ index++ ] = (*argv)[ *iter ];
        }
        *argc = unhandledOptions.size() + 1;
      }
      else
      {
