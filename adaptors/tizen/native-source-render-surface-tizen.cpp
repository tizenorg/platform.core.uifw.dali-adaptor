...
{
  Impl( Dali::PositionSize positionSize, const std::string& name, bool is Transparent )
  ...
  tbm_surface_h mConsumeTbmSurface;
...
NativeSourceRenderSurface::~NativeSourceRenderSurface()
{
  ...
  if( mImpl->mOwnSurface )
  {
    ...
    if( mImpl->mConsumeTbmSurface )
    {
      tbm_surface_queue_release( mImpl->mTbmQueue, mImpl->mConsumeTbmSurface );
    }
...
tbm_surface_h NativeSourceRenderSurface::GetDrawable()
{
  ...
  return mImpl->mConsumeTbmSurface;
}
...
void NativeSourceRenderSurface::InitializeEgl( EglInterface& egl )
{
  ...
  eglImpl.ChooseConfig(false, mImpl->mColorDepth);
}
...
void NativeSourceRenderSurface::PostRender( EglInterface& egl, Integration::GlAbstraction& glAbstraction, DisplayConnection* displayConnection, bool replacingSurface )
{
  ...
  {
    ConditionalWait::ScopedLock lock( mImpl->mTbmSurfaceCondition );
    tbmError = tbm_surface_queue_acquire( mImpl->mTbmQueue, &mImpl->mConsumeSurface );

    if( tbmError != TBM_SURFACE_QUEUE_ERROR_NONE )
    {
      tbm_surface_queue_can_acquire( mImpl->mTbmQueue, 1 );
      tbmError = tbm_surface_queue_acquire( mImpl->mTbmQueue, &mImpl->mConsumeSurface );
...
void NativeSourceRenderSurface::CreateWlRenderable()
{
  ...
  mImpl->mTbmQueue = tbm_surface_queue_create( 3, mPosition.width, mPosition.height, mImpl->mTbmFormat, TBM_BO_DEFAULT );
