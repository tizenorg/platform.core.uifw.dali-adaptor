...
const char* FRAGMENT_SHADER = DALI_COMPOSE_SHADER(
  ...
  void main()\n
  {\n
    gl_FragColor = texture2D( sTexture, vTexCoord.xy ) * uColor;\n
  }\n
);
  ...
  mEglImageExtensions( NULL )
{
  ...
  {
    tizenPlatform->SetNativeSourceFragmentShaderCode( FRAGMENT_SHADER );
  }
...
void NativeImageSource::SetSurface( Any nativeImageSource ) const
{
...
tbm_surface* GetSurfaceFromAny( Any source ) const
{
  ...
  if( source.GetType() == typeid( source ) )
  {
    return static_cast< tbm_surface* >( &source );
  }
  else
  {
    return NULL;
...
tbm_surface_h NativeImageSource::CreateTbmsurface( unsigned int& width, unsigned int& height, int depth )
{
  tbm_format *formats;
  uint32_t format_num;
...
bool NativeImageSource::GetPixels(std::vector<unsigned char>& pixbuf, unsigned& width, unsigned& height, Pixel::Format& pixelFormat) const
{
  if( mTbmsurface != NULL )
  {
    ...
    uint32_t stride = surface_info.stride;
