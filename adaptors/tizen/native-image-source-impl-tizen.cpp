...
bool NativeImageSource::IsSupportedColorDepth( Dali::NativeImageSource::ColorDepth colorDepth )
{
  uint32_t *formats;
  uint32_t format_num;
  tbm_format dali_format = TBM_FORMAT_RGB888;

  switch( colorDepth )
  {
    case Dali::NativeImageSource::COLOR_DEPTH_DEFAULT:
    {
      dali_format = TBM_FORMAT_RGBA8888;
      break;
    }
    case Dali::NativeImageSource::COLOR_DEPTH_8:
    {
      dali_format = TBM_FORMAT_C8;
      break;
    }
    case Dali::NativeImageSource::COLOR_DEPTH_16:
    {
      dali_format = TBM_FORMAT_RGB565;
      break;
    }
    case Dali::NativeImageSource::COLOR_DEPTH_24:
    {
      dali_format = TBM_FORMAT_RGB888;
      break;
    }
    case Dali::NativeImageSource::COLOR_DEPTH_32:
    {
  ...
  DALI_LOG_INFO("dali_format=%d\n", dali_format);

  if (tbm_surface_query_formats (&formats, &format_num))
  {
    for(unsigned int i = 0 ; i < format_num ; i++)
    {
      DALI_LOG_INFO("formats[%d]=%d\n", i, formats[i]);
      if (formats[i] == dali_format)
      {
        free (formats);
        return true;
      }
