...
// Constructor
...
#ifdef APPCORE_WATCH_AVAILABLE
  static void GetWatchTime(watch_time_h time_h, WatchApplication::WatchTime *time)
  {
    watch_time_get_hour(time_h, &time->hour);
    watch_time_get_hour24(time_h, &time->hour24);
    watch_time_get_minute(time_h, &time->minute);
    watch_time_get_second(time_h, &time->second);
    time->extension = time_h;    // Keep time handle for further usage and API modification
  }
  ...
  {
    Observer *observer = &static_cast<Framework*>(data)->mObserver;
    WatchApplication::WatchTime curTime;

    GetWatchTime(time, &curTime);
