...
#ifdef APPCORE_WATCH_AVAILABLE
  void GetWatchTime(watch_time_h time_h, Watch::WatchTime *time)
  {
    watch_time_get_hour(time_h, &time->hour);
    watch_time_get_hour24(time_h, &time->hour24);
    watch_time_get_minute(time_h, &time->min);
    watch_time_get_second(time_h, &time->sec);
    time->watch_time_h = time_h;    // Keep time handle for further usage and API modification
  }
