...
/**
 * @brief Native Dali application class that draws to it's own window.
 *
 * It provides a means for initialising the resources required by
 * Dali::Core.  The Application class emits several signals which the
 * user can connect to.  The user should not create any Dali objects
 * in the main function and instead should connect to the Init signal
 * of the Application and create the Dali objects in the connected
 * callback.
 *
...
{
public:
  typedef SignalV2< void (Application&) > AppSignalV2; ///< General signal type
...
public:
  ...
  /**
   * @brief This is the constructor for applications when a layout for the application is specified.
   *
   * @param[in,out]  argc        A pointer to the number of arguments
   * @param[in,out]  argv        A pointer the the argument list
  ...
  /**
   * @brief This is the constructor for applications with a name and when a layout for the application is specified.
   *
   * @param[in,out]  argc  A pointer to the number of arguments
   * @param[in,out]  argv  A pointer the the argument list
   * @param[in]  name  A name of application
...
public:
  ...
  /**
   * @brief This returns a handle to the Orientation object used by Application.
   *
   * This allows the user to determine the orientation of the device
   * and connect to a signal emitted whenever the orientation changes.
   *
  ...
  /**
   * @brief Returns the local thread's instance of the Application class.
   *
...
public:  // Signals
  ...
  /**
   * @brief The user should connect to this signal to determine when
   * they should initialise their application.
   */
  AppSignalV2& InitSignal();

  /**
   * @brief The user should connect to this signal to determine when
   * they should terminate their application.
   */
  AppSignalV2& TerminateSignal();

  /**
   * @brief The user should connect to this signal if they need to
   * perform any special activities when the application is about to
   * be paused.
   */
  ...
  /**
   * @brief The user should connect to this signal if they need to
   * perform any special activities when the application has resumed.
   */
