...
void ResourceLogger()
{
  ...
  for ( ResourceTrackingManager::ResourceTrackerConstIter TrackIter = g_ResourceTrackingManager->GetStartTrackerList(); TrackIter != g_ResourceTrackingManager->GetEndTrackerList(); ++TrackIter )
  {
    ...
    switch( message.GetMessage() )
    {
      case ResourceTrackMessage::LOAD_BITMAP:
      {
        ...
        if ( message.GetParameter( 0 ) )
        {
          ...
          g_ResourceTrackingManager->AddToCPUTracked( message.GetParameter( 3 ) );
          ...
          g_ResourceTrackingManager->InsertCPUResource( message.GetPointer(), message.GetParameter( 2 ) );
        }
        else
        {
      ...
      {
        g_ResourceTrackingManager->AddToGPUTracked( message.GetParameter( 3 ) );

        // Add this entry to the GPU resources tracker
        g_ResourceTrackingManager->InsertGPUResource( message.GetParameter( 2 ), message.GetPointer(), message.GetParameter( 3 ) );
      ...
      case ResourceTrackMessage::DELETE_TEXTURE:
      {
        ...
        Dali::Internal::Adaptor::ResourceTrackingManager::GPUResourcesIter GIter = g_ResourceTrackingManager->GetGPUResource( message.GetParameter( 0 ) );
        if ( GIter != g_ResourceTrackingManager->GetGPUResourcesEnd() )
        {

          // Take stored size away from running GPU total
          g_ResourceTrackingManager->TakeFromGPUTracked( GIter->second.size );

          // The pointer entry in the GPUslot structure contains the pointer that we can use as a key to lookup the resource ID
          Dali::Internal::Adaptor::ResourceTrackingManager::CPUResourcesIter CIter = g_ResourceTrackingManager->GetCPUResource( GIter->second.ptr );
          if ( CIter != g_ResourceTrackingManager->GetCPUResourcesEnd() )
          {
            ...
            g_ResourceTrackingManager->SetResourceGPUStatus( CIter, false );
            g_ResourceTrackingManager->EraseCPUResource( CIter );
          ...
          g_ResourceTrackingManager->EraseGPUResource( GIter );
        }
        else
        {
      ...
      {
        g_ResourceTrackingManager->TakeFromCPUTracked( message.GetParameter( 3 ) );
        Dali::Internal::Adaptor::ResourceTrackingManager::CPUResourcesIter CIter = g_ResourceTrackingManager->GetCPUResource( message.GetPointer() );
        if ( CIter != g_ResourceTrackingManager->GetCPUResourcesEnd() )
        {
          ...
          g_ResourceTrackingManager->SetResourceCPUStatus( CIter, false );
          g_ResourceTrackingManager->EraseCPUResource( CIter );
        }
        else
        {
      ...
      case ResourceTrackMessage::UPLOAD_TEXTURE:
      {
        ...
        if ( CIter != g_ResourceTrackingManager->GetCPUResourcesEnd() )
        {
          ...
          g_ResourceTrackingManager->SetResourceGPUStatus( CIter, true );
        }
        else
        {
    ...
    if ( foundMessage )
    {
      LOG( LOG_INFO, "DALI", "[Resource Tracker] CPU Total: %iK, GPU Total: %iK, CPU Peak: %iK, GPU Peak: %iK\n\n",
           g_ResourceTrackingManager->GetCPUTracked() / 1024U,
           g_ResourceTrackingManager->GetGPUTracked() / 1024U,
           g_ResourceTrackingManager->GetCPUPeak() / 1024U,
           g_ResourceTrackingManager->GetGPUPeak() / 1024U );
      break;
    }
  }
}
