...
const unsigned int DEFAULT_LOG_FREQUENCEY = 2;
}
                                                                ...
                                                                PerformanceMarker::MarkerFilter type  )
{
  ...
  StatContext* statContext = new StatContext( contextId, name, type,  DEFAULT_LOG_FREQUENCEY  );
...
void StatContextManager::SetLoggingLevel(  unsigned int statisticsLogOptions, unsigned int logFrequency)
{
  ...
  // currently uses DALI_FPS_TRACKING environment variable to determine to log frequency
  // if it's not set it will be zero
  if( logFrequency == 0 )
  {
    logFrequency = DEFAULT_LOG_FREQUENCEY ;
  }
