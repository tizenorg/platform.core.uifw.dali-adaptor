...
PerformanceInterface::ContextId PerformanceServer::AddContext( const char* name )
{
  ...
  mStatContexts.PushBack( StatContextIdPair( new StatContext( name ), contextId ) );
...
void PerformanceServer::LogMarkers( StatContext* context )
{
  ...
  for( StatContext::MarkerVector::SizeType i = 0; i < context->markers.Size(); ++i)
  {
