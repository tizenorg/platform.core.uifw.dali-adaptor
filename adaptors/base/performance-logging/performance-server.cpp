...
{
  // Add defaults
  mUpdateStats = AddContext( "Update" );
  mRenderStats = AddContext( "Render" );
  mEventStats = AddContext( "Event" );
...
PerformanceServer::StatContext::StatContext( std::string contextName )
: name( contextName ),
  logFrequencyMicroseconds( DEFAULT_LOG_FREQUENCEY * MICROSECONDS_PER_SECOND ),
  log( true )
{
...
PerformanceInterface::ContextId PerformanceServer::AddContext( std::string name )
{
...
void PerformanceServer::AddMarkerToLog( const PerformanceMarker& marker, ContextId contextId )
{
  ...
  if( context )
  {
    ...
    {
      std::stringstream ss;
      ss << GetMarkerName( marker.GetType() ) << ":" << context->name;
    ...
    if( marker.GetType() == V_SYNC )
    {
      ...
      if( microseconds >= context->logFrequencyMicroseconds )
      {
        ...
        if( context->log )
        {
          LogMarker( context->name.c_str(), context->stats );
        }
