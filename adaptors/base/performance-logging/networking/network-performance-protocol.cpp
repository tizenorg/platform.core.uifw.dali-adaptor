...
unsigned int GetMaxCommandStringLength( unsigned int bufferLength )
{
  // MAX_COMMAND_STRING_LENGTH includes null terminator
  //
  if( bufferLength > MAX_COMMAND_STRING_LENGTH )
  {
    return MAX_COMMAND_STRING_LENGTH;
  }
  else
  {
    return bufferLength;
...
bool GetCommandId( const char* commandString, unsigned int lengthInBytes, CommandId& commandId, unsigned int& intParam, std::string& stringParam  )
{
  ...
  for( unsigned int i = 0 ; i < CommandLookupLength; ++i )
  {
    if( strncmp( commandString, CommandLookup[i].cmdString ,strlen(CommandLookup[i].cmdString  )) == 0 )
    {
      ...
      else if (CommandLookup[i].paramType == STRING )
      {
        ...
        int count = sscanf(commandString,"%*s %as",&charParam);
        if( count != 1 )
        {
          // missing parameter
          return false;
