...
const unsigned int CONNECTION_BACKLOG = 2;
...
void NetworkPerformanceServer::SendData( const char* data, unsigned int bufferSizeInBytes,unsigned int clientId )
{
  if( ! mClientCount )
  {
    return;
...
void NetworkPerformanceServer::TransmitMarker( const PerformanceMarker& marker )
{
  // prevent clients been added / deleted while transmiting data
  boost::mutex::scoped_lock sharedDatalock( mClientListMutex );
