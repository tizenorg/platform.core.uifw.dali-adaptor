...
bool NetworkPerformanceClient::TransmitMarker( const PerformanceMarker& marker )
{
  ...
  {
    void* buffer[ PerformanceMarker::SERIALIZED_SIZE ];

    marker.Serialize( buffer, sizeof( buffer ));

    return mSocket->Write( buffer, sizeof( buffer ) );
  }
}
