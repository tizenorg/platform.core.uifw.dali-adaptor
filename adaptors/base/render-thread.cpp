...
ChangeSurfaceRequest::ChangeSurfaceRequest(RenderSurface* newSurface)
: RenderRequest(RenderRequest::CHANGE_SURFACE),
  mNewSurface( newSurface )
{
}

RenderSurface* ChangeSurfaceRequest::GetSurface()
{
  return mNewSurface;
}
...
bool RenderThread::Run()
{
  ...
  while( running )
  {
    ...
    bool response = ProcessRequest( request );
    ...
    if( running && PreRender() == true)
    {
      ...
      mUpdateRenderSync.RenderFinished( renderStatus.NeedsUpdate(), response );
...
bool RenderThread::ProcessRequest( RenderRequest* request )
{
  ...
  if( request != NULL )
  {
    ...
    {
      case RenderRequest::CHANGE_SURFACE:
      {
        // change the surface
        ChangeSurfaceRequest* changeSurfaceRequest = static_cast<ChangeSurfaceRequest*>(request);
        ChangeSurface( changeSurfaceRequest->GetSurface() );
        response = true;
        break;
      }
    }
...
void RenderThread::ChangeSurface( RenderSurface* newSurface )
{
  ...
  mSurface->StopRender(); // @todo - check if calling this from render thread is safe
