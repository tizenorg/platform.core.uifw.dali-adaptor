...
void SingleThreadController::Initialize()
{
  ...
  mTimer = Dali::Timer::New( MILLISECONDS_PER_FRAME );
...
void SingleThreadController::RequestUpdateOnce()
{
  if( mState == State::SLEEPING || mState == State::PAUSED )
  {
    ...
    mCore.Update( lastFrameDelta, mLastUpdateRenderTime, mLastUpdateRenderTime + mRefreshRate * MILLISECONDS_PER_FRAME, status );
...
void SingleThreadController::SetRenderRefreshRate(unsigned int numberOfVSyncsPerRender )
{
  if ( numberOfVSyncsPerRender != mRefreshRate )
  {
    DALI_LOG_INFO( gLogFilter, Debug::General, "%s()\n", __FUNCTION__ );

    mRefreshRate = numberOfVSyncsPerRender;
...
void SingleThreadController::UpdateRender( bool incrementTime )
{
  ...
  mFpsTracker.Track( GetTimeSinceLastRender() );
  ...
  if( mState != State::STOPPED )
  {
    ...
    if( renderStatus.HasRendered() && mState != State::STOPPED )
    {
      mRenderHelper.PostRender();
    }
...
float SingleThreadController::GetTimeSinceLastRender()
{
