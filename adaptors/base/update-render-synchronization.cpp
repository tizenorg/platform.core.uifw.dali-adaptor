...
UpdateRenderSynchronization::UpdateRenderSynchronization( AdaptorInternalServices& adaptorInterfaces,
                                                          unsigned int numberOfFramesPerRender)
...
{
  mFrameTime.SetMinimumFrameTimeInterval( mNumberOfFramesPerRender * TIME_PER_FRAME_IN_MICROSECONDS );
  mRunning = true;
}
...
bool UpdateRenderSynchronization::VSyncNotifierSyncWithUpdateAndRender( bool validSync, unsigned int frameNumber, unsigned int seconds, unsigned int microseconds, unsigned int& numberOfFramesPerRender )
{
  if( numberOfFramesPerRender != mNumberOfFramesPerRender )
  {
    numberOfFramesPerRender = mNumberOfFramesPerRender; // save it back
    mFrameTime.SetMinimumFrameTimeInterval( mNumberOfFramesPerRender * TIME_PER_FRAME_IN_MICROSECONDS );
  }
...
void UpdateRenderSynchronization::SetNumberOfFramesPerRender( unsigned int numberOfFramesPerRender )
{
  //@todo Protect me?
  mNumberOfFramesPerRender = numberOfFramesPerRender;
}
...
{
  mFrameTime.PredictNextVSyncTime( mNumberOfFramesPerRender, lastFrameDeltaSeconds, lastVSyncTimeMilliseconds, nextVSyncTimeMilliseconds );
}
