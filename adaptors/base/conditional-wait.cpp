...
ConditionalWait::AtomicScope::AtomicScope( ConditionalWait& wait ) : mWait(wait)
{
  pthread_mutex_lock( &wait.mImpl->mutex );
}

ConditionalWait::AtomicScope::~AtomicScope()
{
  ConditionalWait& wait = mWait;
  pthread_mutex_unlock( &wait.mImpl->mutex );
}

void ConditionalWait::Wait( const AtomicScope& scope )
{
  // Scope must be locked:
  DALI_ASSERT_DEBUG( &scope.GetLockedWait() == this );
  if( &scope.GetLockedWait() != this )
  {
    DALI_LOG_ERROR("AtomicScope passed in does not lock this instance.");
  }
  ...
  // We return with our mutex locked safe in the knowledge that the AtomicScope
