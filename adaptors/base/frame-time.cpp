...
void FrameTime::PredictNextVSyncTime( unsigned int numFramesPerRender, float& lastFrameDeltaSeconds, unsigned int& lastVSyncTimeMilliseconds, unsigned int& nextVSyncTimeMilliseconds )
{
  if ( mRunning )
  {
    ...
    @todo Fix this algorithm to work with numFramesPerRender > 1
