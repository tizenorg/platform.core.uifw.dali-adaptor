...
void CombinedUpdateRenderController::UpdateRenderThread()
{
  ...
  while( UpdateRenderReady() )
  {
    ...
    currentTime = seconds;
    currentTime *= MILLISECONDS_PER_SECOND;
    currentTime += nanoseconds / NANOSECONDS_PER_MILLISECOND;

    frameDelta = ( float )( mNumberOfFramesPerRender * FRAME_DELTA );
    nextSyncTime = ( unsigned int )( currentTime + mNumberOfFramesPerRender * MILLISECONDS_PER_FRAME );
