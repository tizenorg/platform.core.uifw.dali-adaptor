  ...
  mPostRendering( FALSE )
{
  ...
  /**
   * Need a cancel-sleep-trigger to handle a use-case where update/render has sent a sleep-request in the previous frame which causes is to sleep,
   * only for a previous update-request to cause another update/render and at the end of the update/render, we want to carry on updating.
   *
   * This can happen when update/render take a long time and we want to start another animation when a previous animation ends.
   *
   *  1) UPDATE/RENDER THREAD:  Do Update/Render, Updates required, notification to event-thread (animation finished etc.)
   *  2) UPDATE/RENDER THREAD:  Do Update/Render, then no Updates required -> Sleep Trigger
   *  3) MAIN THREAD:           Update Request: COUNTER = 2
   *  4) UPDATE/RENDER THREAD:  Do Update/Render, then no Updates required -> Sleep Trigger
   *  5) MAIN THREAD:           Sleep Request (from 2): COUNTER = 1
   *  6) MAIN THREAD:           Sleep Request (from 4): COUNTER = 0 -> Go to sleep
   *  7) UPDATE/RENDER THREAD:  Do Update/Render, Updates required (Without the cancel-sleep-trigger, we would not do any more updates after this)
   *                                              Now, we do a cancel-sleep-trigger
   *  8) MAIN THREAD:           Update Request (Cancel Sleep Trigger): COUNTER = 1
   *  9) UPDATE/RENDER THREAD:  Do Update/Render, Updates required
   *  ...
   *
   ...
   *
   * Still ensures we preserve battery life as we do a cancel sleep trigger ONLY if updates are required.
   */
...
void CombinedUpdateRenderController::UpdateRenderThread()
{
  ...
  while( UpdateRenderReady() )
  {
    ...
    const unsigned int noOfFramesSinceLastUpdate = std::max( timeSinceLastFrame / mDefaultFrameDurationNanoseconds, static_cast< uint64_t >( 1 ) );
