...
: mFpsTracker( environmentOptions ),
  ...
  mLastFrameTimeSeconds( 0u ),
  mLastFrameTimeNanoseconds( 0u ),
  mNumberOfFramesPerRender( 1 ),
  mTryToSleepCount( 0u ),
  mRunning( FALSE ),
...
void CombinedUpdateRenderController::UpdateRenderThread()
{
  ...
  uint64_t defaultFrameTime = 0;

  // Update time
  mPlatformAbstraction.GetTimeNanoseconds( mLastFrameTimeSeconds, mLastFrameTimeNanoseconds );
  ...
  while( UpdateRenderReady() )
  {
    ...
    currentFrameTime *= MILLISECONDS_PER_SECOND;
    currentFrameTime += nanoseconds / NANOSECONDS_PER_MILLISECOND;

    frameDelta = ( float )( mNumberOfFramesPerRender * FRAME_DELTA );
    nextFrameTime = ( unsigned int )( currentFrameTime + mNumberOfFramesPerRender * MILLISECONDS_PER_FRAME );
    ...
    mCore.Update( frameDelta, currentFrameTime, nextFrameTime, updateStatus );
    ...
    {
      uint64_t timeSinceLastFrame = seconds - mLastFrameTimeSeconds;
      timeSinceLastFrame *= NANOSECONDS_PER_SECOND;

      int64_t nanosecondsDiff = nanoseconds - mLastFrameTimeNanoseconds;
      timeSinceLastFrame += nanosecondsDiff;

      float absoluteTimeSinceLastRender = ( float )( ( timeSinceLastFrame ) * NANOSECONDS_TO_SECOND );
    ...
    mLastFrameTimeSeconds = seconds;
    mLastFrameTimeNanoseconds = nanoseconds;

    mPlatformAbstraction.GetTimeNanoseconds( seconds, nanoseconds );

    uint64_t frameTime = seconds - mLastFrameTimeSeconds;
    frameTime *= NANOSECONDS_PER_SECOND;

    int64_t nanosecondsDiff = nanoseconds - mLastFrameTimeNanoseconds;
    frameTime += nanosecondsDiff;

    defaultFrameTime = mNumberOfFramesPerRender * TIME_PER_FRAME_IN_NANOSECONDS;
