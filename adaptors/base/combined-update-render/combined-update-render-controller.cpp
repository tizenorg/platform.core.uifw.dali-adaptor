...
void CombinedUpdateRenderController::UpdateRenderThread()
{
  ...
  while( UpdateRenderReady() )
  {
    ...
    currentTime = ( uint64_t )( ( seconds * MICROSECONDS_PER_SECOND ) + microSeconds );

    absoluteTimeSinceLastRender = ( float )( ( currentTime - mLastRenderTime ) * MICROSECONDS_TO_SECOND );
    mLastRenderTime = currentTime;

    frameDelta = ( float )( mNumberOfVSyncsPerRender * FRAME_DELTA );

    currentTime /= MICROSECONDS_PER_MILLISECOND;
    nextSyncTime = ( unsigned int )( currentTime + mNumberOfVSyncsPerRender * MILLISECONDS_PER_FRAME );
