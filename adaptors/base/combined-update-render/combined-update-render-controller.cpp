...
/**
 * Needs to be 2 as if it was 1 (or if a count was not used), then there is a danger that, on the event thread we could have:
 ...
 * Using a counter ensures we do not pause the update/render thread too early.
 */
...
void CombinedUpdateRenderController::SetRenderRefreshRate( unsigned int numberOfFramesPerRender )
{
  ...
  LOG_EVENT( "mDefaultFrameDelta(%.6f), mDefaultFrameDurationMilliseconds(%u), mDefaultFrameDurationNanoseconds(%u)", mDefaultFrameDelta, mDefaultFrameDurationMilliseconds, mDefaultFrameDurationNanoseconds);
}
...
void CombinedUpdateRenderController::UpdateRenderThread()
{
  ...
  while( UpdateRenderReady() )
  {
    ...
    unsigned int currentFrameDuration = currentFrameEndTime - currentFrameStartTime;

    if( currentFrameDuration < mDefaultFrameDurationNanoseconds )
    {
      timespec sleepTime;
      sleepTime.tv_sec = 0;
      sleepTime.tv_nsec = mDefaultFrameDurationNanoseconds - currentFrameDuration;

      // Sleep for the specified duration.
      // Using a while loop in case of interrupt which means we should sleep again (the remaining time is passed back to us)
      timespec remainingTime;
      while( nanosleep( &sleepTime, &remainingTime ) && errno == EINTR )
      {
