...
private:
  ...
  uint64_t                          mLastFrameTimeSeconds;           ///< The seconds part of the last frame time.
  uint64_t                          mLastFrameTimeNanoseconds;       ///< The nanoseconds part of the last frame time.

  unsigned int                      mNumberOfFramesPerRender;         ///< How many frames for each update/render cycle. Not protected by lock, but written to rarely so not worth adding a lock when reading.
