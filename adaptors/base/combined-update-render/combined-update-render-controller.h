...
private:
   ...
   */
  static inline void* InternalVSyncThreadEntryFunc( void* This )
  {
  ...
  /**
   * Checks if the Update/Render thread should be running.
   * This will lock the mutex in mUpdateRenderThreadWaitCondition.
   *
   * @return true if running, false otherwise.
   */
  inline bool IsUpdateRenderThreadRunning();
   ...
   * @param[in] This A pointer to the current object
   */
  static inline void* InternalUpdateRenderThreadEntryFunc( void* This )
  {
...
private:
  ...
  volatile unsigned int             mNumberOfThreadsStarted;          ///< The number of threads that are initialised & running (set by v-sync & the update/render thread, read by event-thread).
  ...
  volatile unsigned int             mVSyncThreadStop;                 ///< Whether the V-Sync thread should be stopped (set by the event-thread, read by the v-sync-thread).

  volatile unsigned int             mUpdateRenderThreadStop;          ///< Whether the Update/Render thread be stopped (set by the event-thread, read by the update-render-thread).
  ...
  volatile unsigned int             mSurfaceReplaced;                 ///< Whether the surface has been replaced (set by the event & update-render, read by event-thread).
