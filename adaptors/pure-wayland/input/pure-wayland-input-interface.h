 ...
 *
 * The events that come from keyboard / pointer and touch devices and are from a specific seat.
...
public: // Helper functions used to find the seat associated with the keyboard/pointer/touch device
   ...
   */
  virtual Seat* GetSeat( const WlKeyboard* keyboard ) const = 0;
   ...
   */
  virtual Seat* GetSeat( const WlPointer* pointer ) const = 0;
   ...
   */
  virtual Seat* GetSeat( const WlTouch* touch ) const = 0;
   ...
   * @return the seat the touch device belongs to
   */
  virtual Seat* GetSeat( const WlSeat* seat ) const = 0;
...
protected:
   ...
   * Virtual destructor
   */
  virtual ~WaylandInputInterface()
  {
  }

  // Undefined copy constructor.
  WaylandInputInterface( const WaylandInputInterface& );

  // Undefined assignment operator.
  WaylandInputInterface& operator=( const WaylandInputInterface& );
};
