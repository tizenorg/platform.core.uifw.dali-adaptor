...
void Window::SetEffect(const std::string& effect)
{
  ...
  {
    std::stringstream ss;
    ss << mEnabledEffects.size() << ":" << effect << EFFECT_ENABLE_POSTFIX;
    Ecore_X_Window ecoreWindow = x11Window->GetXWindow();
...
{
  if (mEnabledEffects.empty())
  {
    DALI_ASSERT_ALWAYS("No enabled effect on the window.");
  }

  std::vector<std::string>::iterator iter = std::find(mEnabledEffects.begin(), mEnabledEffects.end(), effect);
  DALI_ASSERT_ALWAYS(iter != mEnabledEffects.end() && "This effect isn't enabled.");

  ECore::WindowRenderSurface* x11Window = dynamic_cast< ECore::WindowRenderSurface * >( mSurface );
  if (x11Window)
  {
    std::stringstream ss;
    ss << iter - mEnabledEffects.begin() << ":" << effect << EFFECT_DISABLE_POSTFIX;
    Ecore_X_Window ecoreWindow = x11Window->GetXWindow();
    ecore_x_window_prop_property_set(ecoreWindow, ECORE_X_ATOM_E_WINDOW_AUX_HINT,
                                     ECORE_X_ATOM_STRING, 8,
                                     (void*)ss.str().c_str(), ss.str().size() + 1);
...
void Window::RemoveAllEffects()
{
  mEnabledEffects.clear();

  ECore::WindowRenderSurface* x11Window = dynamic_cast< ECore::WindowRenderSurface * >( mSurface );
  if (x11Window)
  {
    Ecore_X_Window ecoreWindow = x11Window->GetXWindow();
    ecore_x_window_prop_property_del(ecoreWindow, ECORE_X_ATOM_E_WINDOW_AUX_HINT);
  }
}
