...
{
  // if TBM surface has been created outside of native Image we can return
  if (mTBMSurface)
  {
  ...
  // Create Tbm Surface
  mTBMSurface = tbm_surface_create(mWidth, mHeight, TBM_FORMAT_RGBA8888);
  if (NULL == mTBMSurface)
  {
...
bool OffscreenImage::GetPixels(std::vector<unsigned char>& pixbuf, unsigned& width, unsigned& height, Pixel::Format& pixelFormat) const
{
  ...
  if(!success)
  {
    DALI_LOG_ERROR("Failed to get pixels from OffscreenImage.");
    pixbuf.resize(0);
    width = 0;
    height = 0;
  }
...
bool OffscreenImage::EncodeToFile(const std::string& filename) const
{
  ...
  Pixel::Format pixelFormat = Pixel::BGRA8888;
...
int OffscreenImage::GetPixelDepth(Dali::OffscreenImage::ColorDepth depth) const
{
  switch (depth)
  {
    case Dali::OffscreenImage::COLOR_DEPTH_DEFAULT:
    {
      // Get the default screen depth
      return ecore_x_default_depth_get(ecore_x_display_get(), ecore_x_default_screen_get());
    }
    case Dali::OffscreenImage::COLOR_DEPTH_8:
    {
      return 8;
    }
    case Dali::OffscreenImage::COLOR_DEPTH_16:
    {
      return 16;
    }
    case Dali::OffscreenImage::COLOR_DEPTH_24:
    {
      return 24;
    }
    case Dali::OffscreenImage::COLOR_DEPTH_32:
    {
      return 32;
    }
    default:
...
Ecore_X_Pixmap OffscreenImage::GetPixmapFromAny(Any pixmap) const
{
  if (pixmap.Empty())
  {
    return 0;
