...
struct EventHandler::Impl
{
      ...
      if( XQueryExtension( display, "XInputExtension", &opcode, &event, &error ) )
      {
        ...
        if( XIQueryVersion( display, &major, &minor ) != BadRequest )
        {
          ...
          if( match )
          {
            ...
            if( ret == 0 )
            {
              // Register rotary wheel events
              mEcoreEventHandler.push_back( ecore_event_handler_add( ECORE_X_EVENT_GENERIC, EcoreEventRotaryWheel, handler ) );
            }
            else
            {
  ...
  /**
   * Called when a rotary wheel is received.
   */
  static Eina_Bool EcoreEventRotaryWheel( void* data, int type, void* event )
  {
    ...
    switch( genericEvent->evtype )
    {
      case XI_RawMotion:
      {
        ...
        if( XIMaskIsSet( xiRawEvent->valuators.mask, 2) )
        {
          ...
          DALI_LOG_INFO( gImfLogging, Debug::General, "EVENT EcoreEventRotaryWheel: z: %d\n", z );
