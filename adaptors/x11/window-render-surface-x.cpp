...
void WindowRenderSurface::PostRender( EglInterface& egl, Integration::GlAbstraction& glAbstraction, DisplayConnection* dc, unsigned int deltaTime, bool replacingSurface )
{
  Internal::Adaptor::EglImplementation& eglImpl = static_cast<Internal::Adaptor::EglImplementation&>( egl );
  eglImpl.SwapBuffers();
  ...
  if(mNeedToApproveDeiconify)
  {
    ...
    XDisplay* display = AnyCast<XDisplay *>(dc->GetDisplay());
