...
// Need to define own method as the log function is not installed when this object is created so no logging is shown with DALI_LOG_INFO
#define DALI_LOG_SINGLETON_SERVICE_NO_LOG_FUNCTION(level, message)                        \
...
: mSingletonContainer()
{
  ...
  DALI_LOG_SINGLETON_SERVICE_NO_LOG_FUNCTION( Debug::Concise, "SingletonService Created\n" );
}

SingletonService::~SingletonService()
{
  gSingletonService.release();
  DALI_LOG_SINGLETON_SERVICE_NO_LOG_FUNCTION( Debug::Concise, "SingletonService Destroyed\n" );
}
