 ...
 * @return buffer with the contents of the file Dali::Vector
 *
 * @note: User is reponsible for freeing the buffer.
 * @note: If we can't load the file the function will return NULL
 *
 */
DALI_IMPORT_API Dali::Vector<char> ReadFile(const std::string& filename, FileLoader::FileType fileType = BINARY);
 ...
 * @param[in] fileType  How we want to load the file. Binary or Text. Binary default
 * @return buffer with the contents of the file in a Dali::Vector
 *
 * @note: User is reponsible for freeing the buffer.
 * @note: If we can't load the file the function will return NULL
 *
 */
DALI_IMPORT_API Dali::Vector<char> ReadFile(const std::string& filename, std::streampos& fileSize , FileLoader::FileType fileType = BINARY);
