...
{
  //Integration::BitmapPtr scaled = DownscaleBitmap( Integration::Bitmap& bitmap, const ImageAttributes& requestedAttributes, const std::string& path )
  DALI_TEST_EQUALS( "", "Test not implemented.", TEST_LOCATION );
  END_TEST;
}
...
void DoResizeTest( uint32_t pixels[], const unsigned inputWidth, const unsigned inputHeight, const unsigned int desiredWidth, const unsigned int desiredHeight, const unsigned int expectedWidth, const unsigned int expectedHeight, const char * const location )
{
...
void DoResizeTest( uint32_t pixels[], const unsigned inputWidth, const unsigned inputHeight, const unsigned int desiredWidth, const unsigned int desiredHeight, const char * const location )
{
  DoResizeTest( pixels, inputWidth, inputHeight, desiredWidth, desiredHeight, desiredWidth, desiredHeight, location );
}

void DoResizeTestSequence( uint32_t pixels[], const unsigned int xOffsetIn, const unsigned int yOffsetIn, const unsigned int xOffsetOut, const unsigned int yOffsetOut, const char * const location )
{
  DoResizeTest(pixels, 600 + xOffsetIn, 600 + yOffsetIn, 75 + xOffsetOut, 75 + yOffsetOut, location );
  DoResizeTest(pixels, 512 + xOffsetIn, 512 + yOffsetIn, 16 + xOffsetOut, 16 + yOffsetOut, location );
  DoResizeTest(pixels, 512 + xOffsetIn, 256 + yOffsetIn, 16 + xOffsetOut, 8 + yOffsetOut, location );
  DoResizeTest(pixels, 256 + xOffsetIn, 512 + yOffsetIn, 8 + xOffsetOut, 16 + yOffsetOut, location );
  DoResizeTest(pixels, 256 + xOffsetIn, 512 + yOffsetIn, 4 + xOffsetOut, 8 + yOffsetOut, location );
  DoResizeTest(pixels, 256 + xOffsetIn, 512 + yOffsetIn, 2 + xOffsetOut, 4 + yOffsetOut, location );
  DoResizeTest(pixels, 512 + xOffsetIn, 128 + yOffsetIn, 16 + xOffsetOut, 4 + yOffsetOut, location );
  DoResizeTest(pixels, 512 + xOffsetIn, 64 + yOffsetIn, 16 + xOffsetOut, 2 + yOffsetOut, location );
}
...
int UtcDaliImageOperationsDownscaleInPlacePow2RGBA8888(void)
{
  ...
  DoResizeTestSequence(white_600_x_600, 0, 0, 0, 0, TEST_LOCATION );

  // Test downscales with odd multiples, over by one or three
  DoResizeTestSequence(white_600_x_600, 1, 0, 0, 0, TEST_LOCATION );
  DoResizeTestSequence(white_600_x_600, 0, 1, 0, 0, TEST_LOCATION );
  DoResizeTestSequence(white_600_x_600, 3, 0, 0, 0, TEST_LOCATION );
  DoResizeTestSequence(white_600_x_600, 0, 3, 0, 0, TEST_LOCATION );
  DoResizeTestSequence(white_600_x_600, 1, 3, 0, 0, TEST_LOCATION );
  DoResizeTestSequence(white_600_x_600, 3, 1, 0, 0, TEST_LOCATION );

  // Test downscales with odd multiples, under by one:
  DoResizeTest(white_600_x_600, 63, 31, 7, 3, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 63, 31, 4, 2, 7, 3, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 15, 127, 4, 32, 7, 63, TEST_LOCATION );

  // Test downscales between sizes made up only of prime factors:
  DoResizeTest(white_600_x_600, 11 * 13/*143*/, 17 * 19/*323*/, 3 * 5 /*15*/, 3 * 7 /*21*/, 17 /* 3 truncating halvings */, 40 /* 3 truncating halvings */, TEST_LOCATION );

  // Test downscales to the same size:
  DoResizeTest(white_600_x_600, 600, 600, 600, 600, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 128, 512, 128, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 17, 1001, 17, 1001, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 2, 2, 2, 2, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 11 * 13, 17 * 19, 11 * 13, 17 * 19, 11 * 13, 17 * 19, TEST_LOCATION );

  // Test downscales that request a larger size:
  DoResizeTest(white_600_x_600, 300, 300, 600, 600, 300, 300, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 3, 127, 99, 599, 3, 127, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 600, 600, 999, 999, 600, 600, TEST_LOCATION ); //< checks no out of bounds mem access in this case

  // Test downscales to zero in one or more dimensions:
  DoResizeTest(white_600_x_600, 512, 512, 0, 0, 1, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 256, 0, 0, 2, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 128, 0, 0, 4, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 16,  0, 0, 32, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 128, 512, 0, 0, 1, 4, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 32,  512, 0, 0, 1, 16, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 8,   512, 0, 0, 1, 64, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 2,   512, 0, 0, 1, 256, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 11 * 13/*143*/, 17 * 19/*323*/, 0, 3 * 7 /*21*/, 17 /* 3 truncating halvings */, 40 /* 3 truncating halvings */, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 11 * 13/*143*/, 17 * 19/*323*/, 3 * 5 /*15*/, 0, 17 /* 3 truncating halvings */, 40 /* 3 truncating halvings */, TEST_LOCATION );

  // Test downscales to 1 in one or both dimensions:
  DoResizeTest(white_600_x_600, 512, 512, 1, 1, 1, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 32, 16, 1, 16, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 32, 7, 1, 16, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 32, 7, 1, 16, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 32, 5, 1, 16, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 512, 32, 3, 1, 16, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 32, 512, 1, 1, 1, 16, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 32, 512, 1, 16, 1, 16, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 32, 512, 1, 3, 1, 16, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 33, 33, 1, 1, 1, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 17*19, 17*19, 1, 1, 1, 1, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 33, 33, 3, 1, 4, 4, TEST_LOCATION );
  DoResizeTest(white_600_x_600, 33, 9, 3, 1, 4, 1, TEST_LOCATION );

  DALI_TEST_EQUALS( 8 / 2, 2 * 2, TEST_LOCATION );
...
int UtcDaliImageOperationsAverageScanlines(void)
{
  // Red and cyan, averaging to grey:
  ...
  AverageScanlines( shortEven1, shortEven2, outputBuffer, sizeof(shortEven1) );
  ...
  AverageScanlines( (const unsigned char*) &scanline1[0], (const unsigned char*) &scanline2[0], (unsigned char*) &output[0], scanlineLength * 4 );
...
int UtcDaliImageOperationsAverageScanlinesRGBA8888(void)
{
  ...
  AverageScanlinesRGBA8888( (const unsigned char*) &scanline1[0], (const unsigned char*) &scanline2[0], &output[0], scanlineLength );
...
int UtcDaliImageOperationsAverageScanlinesRGB565(void)
{
  ...
  Dali::Internal::Platform::AverageScanlinesRGB565( shortEven1, shortEven2, outputBuffer,  arrayLength );
