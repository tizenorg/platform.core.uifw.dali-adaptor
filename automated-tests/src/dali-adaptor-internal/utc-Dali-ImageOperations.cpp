...
Dali::IntrusivePtr<Dali::RefCountedVector<uint32_t> > MakeCheckerboardImageRGBA8888( unsigned int width,  unsigned int height, unsigned int checkerSize )
{
  ...
  for( unsigned int cy = 0; cy < height; ++cy )
  {
    ...
    for( unsigned int cx = 0; cx < width; ++cx )
    {
      ...
      //checkColor = RandomInRange(4294967290);
      ...
      for( unsigned int py = 0; py < checkerSize; ++py )
      {
        ...
        for( unsigned int px = 0; px < checkerSize; ++px )
        {
          if(cx == 0 && cy == 0)
          {
            checkLine[px] = 0xff0000ff;// Red
          }
          else if(cx == 7 && cy == 0)
          {
            checkLine[px] = 0xff00ff00;// Green
          }
          else if(cx == 7 && cy == 7)
          {
            checkLine[px] = 0xffff0000;// blue
          }
          else{
            checkLine[px] = checkColor;
          }
        }
      }
    }
  ...
  // Dump checkerboard to disk:
  if( false )
  {
    std::stringstream filename;
    //filename << "/sdcard/image_dumps/MakeCheckerboardImageRGBA8888-" << width << "x" << height << "-" << checkerSize << "-" << ((ptrdiff_t) &image->GetVector()) << ".ppm";
    filename << "/tmp/MakeCheckerboardImageRGBA8888-" << imageWidth << "x" << imageHeight << "-" << checkerSize << /*"-" << ((ptrdiff_t) &image->GetVector()) << */ ".ppm";
    FILE* file = fopen(filename.str().c_str(), "w+");

    if (file != NULL)
    {
      const unsigned imgBufSize = image->GetVector().Size();
      const uint8_t* const buf = (const uint8_t* const) &image->GetVector()[0];
      //Dali::DaliVector<uint8_t> rgb;
      std::vector<uint8_t> rgb;
      rgb.reserve(imgBufSize * 3);
      for( unsigned i = 0; i < imgBufSize * 4; i += 4 )
      {
        rgb.push_back(buf[i]);
        rgb.push_back(buf[i + 1]);
        rgb.push_back(buf[i + 2]);
      }
      fprintf(file, "P6 %u %u 255\n", imageWidth, imageHeight );
      fwrite(&rgb[0], 1, rgb.size(), file);
      fflush(file);
      fclose(file);
    }
  }
...
int UtcDaliImageOperationsPointSampleRGBA888InBounds(void)
{
  ...
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage, inputWidth, inputHeight, (unsigned char*) outputImage, desiredWidth, desiredHeight );
...
int UtcDaliImageOperationsPointSampleCheckerboardRGBA888(void)
{
  ...
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) &image->GetVector()[0], 256, 256, (unsigned char*) outputImage, desiredWidth, desiredHeight );
...
int UtcDaliImageOperationsPointSampleRGBA888ScaleToSinglePixel(void)
{
  ...
  // 1x1 -> 1x1:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,    1,    1, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
  DALI_TEST_EQUALS( outputImage, inputImage[0], TEST_LOCATION );
  outputImage = 0;

  // Single-pixel wide tall stripe:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,    1, 1024, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
  DALI_TEST_EQUALS( outputImage, inputImage[0], TEST_LOCATION );
  outputImage = 0;

  // Single-pixel tall, wide strip:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage, 1024,    1, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
  DALI_TEST_EQUALS( outputImage, inputImage[0], TEST_LOCATION );
  outputImage = 0;

  // Square mid-size image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,  103,  103, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
  DALI_TEST_EQUALS( outputImage, inputImage[0], TEST_LOCATION );
  outputImage = 0;

  // Wide mid-size image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,  313,  79, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
  DALI_TEST_EQUALS( outputImage, inputImage[0], TEST_LOCATION );
  outputImage = 0;

  // Tall mid-size image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,   53,  467, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
  DALI_TEST_EQUALS( outputImage, inputImage[0], TEST_LOCATION );
  outputImage = 0;

  // 0 x 0 input image (make sure output not written to):
  outputImage = 0xDEADBEEF;
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,    0,    0, (unsigned char*) &outputImage, desiredWidth, desiredHeight );
...
int UtcDaliImageOperationsPointSampleRGBA888ScaleToZeroDims(void)
{
  ...
  // 1x1 -> 1x1:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,    1,    1, (unsigned char*) &outputImage, 0, 0 );

  // Single-pixel wide tall stripe:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,    1, 1024, (unsigned char*) &outputImage, 0, 33 );

  // Single-pixel tall, wide strip:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage, 1024,    1, (unsigned char*) &outputImage, 0, 67 );

  // Square mid-size image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,  103,  103, (unsigned char*) &outputImage, 21, 0 );

  // Wide mid-size image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,  313,  79, (unsigned char*) &outputImage, 99, 0 );

  // Tall mid-size image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,   53,  467, (unsigned char*) &outputImage, 9999, 0 );

  // 0 x 0 input image:
  Dali::Internal::Platform::PointSampleRGBA8888( (const unsigned char *) inputImage,    0,    0, (unsigned char*) &outputImage, 200, 99 );
