...
/**
 * @brief Generate a uniform random integer between zero and the parameter passed in.
 **/
uint32_t RandomInRange( uint32_t max )
{
  // Floating point alternative to below: const uint32_t integerRand = floor( drand48() * (max + 0.999999) );
  uint64_t uniformRandom;
  while( (uniformRandom = lrand48()) == 0 ) { /* A zero will underflow in our next step, so avoid it. */ }
  const uint64_t integerRand = ( (uniformRandom - 1) * (max + 1) ) >> 31u;
  return integerRand;
}
...
/**
 * @brief Test the function RandomInRange() used elsewhere in this test suite.
 */
int UtcDaliImageOperationsSelfTestRandomInRange(void)
{
  const uint32_t rangeMax = 255;
  const int numInts = 2000;
  srand48( 99 );
  std::vector<uint32_t> randomInts;
  for( int i = 0; i < numInts; ++i )
  {
    randomInts.push_back( RandomInRange( rangeMax ) );
  }

  // Check all values in range were generated:
  for( uint32_t i = 0; i <= rangeMax; ++i )
  {
    DALI_TEST_CHECK( randomInts.end() != std::find( randomInts.begin(), randomInts.end(), i ) );
  }

  // Check all values generated were in range:
  for( int i = 0; i < numInts; ++i )
  {
    DALI_TEST_CHECK( randomInts[i] <= rangeMax );
  }
...
void DoResizeTestSequenceRGBA8888( uint32_t pixels[], unsigned int xOffsetIn, unsigned int yOffsetIn, unsigned int xOffsetOut, unsigned int yOffsetOut, const char * const location )
{
...
void DoResizeTestSequenceRGB565( uint16_t pixels[], unsigned int xOffsetIn, unsigned int yOffsetIn, unsigned int xOffsetOut, unsigned int yOffsetOut, const char * const location )
{
