...
/**
 * @brief RGBA888 Pixels with random color components.
 */
...
/**
 * @brief RGB565 Pixels with random color components.
 */
...
/**
 * @brief Build a bitmap, downscale it and assert it has the right dimensions.
 */
...
/**
 * @brief Test the top-level function for reducing the dimension of a bitmap of
 * one of the five pixel formats output by image loaders.
 */
 ...
 */
void DoResizeTestRGBA8888( uint32_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, unsigned int expectedWidth, unsigned int expectedHeight, const char * const location )
{
 ...
 */
void DoResizeTestRGB565( uint16_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, unsigned int expectedWidth, unsigned int expectedHeight, const char * const location )
{
  unsigned int resultingWidth = -1, resultingHeight = -1;
 ...
 */
void DoResizeTest2ComponentPair( uint8_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, unsigned int expectedWidth, unsigned int expectedHeight, const char * const location )
{
  unsigned int resultingWidth = -1, resultingHeight = -1;
 ...
 */
void DoResizeTestSingleComponent( uint8_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, unsigned int expectedWidth, unsigned int expectedHeight, const char * const location )
{
  unsigned int resultingWidth = -1, resultingHeight = -1;
...
/**
 * @brief Wrapper for the case where the desired dimensions are expected to be scaled-to exactly.
 */
void DoResizeTestRGBA8888( uint32_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, const char * const location )
{
 ...
 * @brief Wrapper for the case where the desired dimensions are expected to be scaled-to exactly.
 */
void DoResizeTestRGB565( uint16_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, const char * const location )
{
 ...
 * @brief Wrapper for the case where the desired dimensions are expected to be scaled-to exactly.
 */
void DoResizeTest2ComponentPair( uint8_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, const char * const location )
{
 ...
 * @brief Wrapper for the case where the desired dimensions are expected to be scaled-to exactly.
 */
void DoResizeTestSingleComponent( uint8_t pixels[], unsigned inputWidth, unsigned inputHeight, unsigned int desiredWidth, unsigned int desiredHeight, const char * const location )
{
  DoResizeTestSingleComponent( pixels, inputWidth, inputHeight, desiredWidth, desiredHeight, desiredWidth, desiredHeight, location );
}

/**
 * @brief Perform a series of resize tests with different sized input images.
 * Each test essentially asserts that the output image has the expected
 * dimensions after downscaling towards a given target.
 *
 * Apply a small x and y perturbation to the dimensions of the input images and
 * verify that the resulting output images have sizes that are not affected by
 * the small differences in starting image dimensions due to the repeated
 * rounding to an even size that happens at each downscaling step.
 */
void DoResizeTestSequenceRGBA8888( uint32_t pixels[], unsigned int xOffsetIn, unsigned int yOffsetIn, const char * const location )
{
  DoResizeTestRGBA8888( pixels, 600 + xOffsetIn, 600 + yOffsetIn, 75, 75, location );
  DoResizeTestRGBA8888( pixels, 512 + xOffsetIn, 512 + yOffsetIn, 16, 16, location );
  DoResizeTestRGBA8888( pixels, 512 + xOffsetIn, 256 + yOffsetIn, 16, 8, location );
  DoResizeTestRGBA8888( pixels, 256 + xOffsetIn, 512 + yOffsetIn, 8, 16, location );
  DoResizeTestRGBA8888( pixels, 256 + xOffsetIn, 512 + yOffsetIn, 4, 8, location );
  DoResizeTestRGBA8888( pixels, 256 + xOffsetIn, 512 + yOffsetIn, 2, 4, location );
  DoResizeTestRGBA8888( pixels, 512 + xOffsetIn, 128 + yOffsetIn, 16, 4, location );
  DoResizeTestRGBA8888( pixels, 512 + xOffsetIn, 64 + yOffsetIn, 16, 2, location );
}

/**
 * @copydoc DoResizeTestSequenceRGBA8888
 */
void DoResizeTestSequenceRGB565( uint16_t pixels[], unsigned int xOffsetIn, unsigned int yOffsetIn, const char * const location )
{
  DoResizeTestRGB565( pixels, 600 + xOffsetIn, 600 + yOffsetIn, 75, 75, location );
  DoResizeTestRGB565( pixels, 512 + xOffsetIn, 512 + yOffsetIn, 16, 16, location );
  DoResizeTestRGB565( pixels, 512 + xOffsetIn, 256 + yOffsetIn, 16, 8, location );
  DoResizeTestRGB565( pixels, 256 + xOffsetIn, 512 + yOffsetIn, 8, 16, location );
  DoResizeTestRGB565( pixels, 256 + xOffsetIn, 512 + yOffsetIn, 4, 8, location );
  DoResizeTestRGB565( pixels, 256 + xOffsetIn, 512 + yOffsetIn, 2, 4, location );
  DoResizeTestRGB565( pixels, 512 + xOffsetIn, 128 + yOffsetIn, 16, 4, location );
  DoResizeTestRGB565( pixels, 512 + xOffsetIn, 64 + yOffsetIn, 16, 2, location );
}
...
int UtcDaliImageOperationsDownscaleInPlacePow2RGBA8888(void)
{
  ...
  DoResizeTestSequenceRGBA8888( white_600_x_600, 0, 0, TEST_LOCATION );

  // Test downscales with odd multiples, over by one or three
  DoResizeTestSequenceRGBA8888( white_600_x_600, 1, 0, TEST_LOCATION );
  DoResizeTestSequenceRGBA8888( white_600_x_600, 0, 1, TEST_LOCATION );
  DoResizeTestSequenceRGBA8888( white_600_x_600, 3, 0, TEST_LOCATION );
  DoResizeTestSequenceRGBA8888( white_600_x_600, 0, 3, TEST_LOCATION );
  DoResizeTestSequenceRGBA8888( white_600_x_600, 1, 3, TEST_LOCATION );
  DoResizeTestSequenceRGBA8888( white_600_x_600, 3, 1, TEST_LOCATION );
...
int UtcDaliImageOperationsDownscaleInPlacePow2RGB565(void)
{
  ...
  DoResizeTestSequenceRGB565( white_600_x_600, 1, 1, TEST_LOCATION );
  DoResizeTestSequenceRGB565( white_600_x_600, 3, 3, TEST_LOCATION );
  DoResizeTestSequenceRGB565( white_600_x_600, 7, 7, TEST_LOCATION );
  ///@note: No need to be as comprehensive as with RGB888 and RGBA8888 as a template function running the core logic is shared by all.
