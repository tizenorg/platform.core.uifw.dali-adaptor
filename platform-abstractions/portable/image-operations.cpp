...
// A maximum size limit for newly created bitmaps (65535 is chosen as we are using 16bit words for dimensions).
const Vector2 MAXIMUM_TARGET_BITMAP_SIZE( 65535, 65535 );
...
BitmapPtr CropAndPadForFittingMode( BitmapPtr bitmap, ImageDimensions desiredDimensions, FittingMode::Type fittingMode )
{
  ...
  else if( inputWidth != desiredDimensions.GetWidth() || inputHeight != desiredDimensions.GetHeight() )
  {
    ...
    if( scanlinesToCrop != 0 || columnsToCrop != 0 )
    {
      ...
      if( ( desiredWidth > MAXIMUM_TARGET_BITMAP_SIZE.width ) || ( desiredHeight > MAXIMUM_TARGET_BITMAP_SIZE.height ) ||
          ( columnsToPad > MAXIMUM_TARGET_BITMAP_SIZE.width ) || ( scanlinesToPad > MAXIMUM_TARGET_BITMAP_SIZE.height ) )
      {
