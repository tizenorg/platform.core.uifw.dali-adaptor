 ...
 */
BoxDimensionTest DimensionTestForScalingMode( ScalingMode scalingMode )
{
 ...
 */
ImageDimensions FitToScalingMode( ImageDimensions requestedSize, ImageDimensions sourceSize, ScalingMode scalingMode )
{
...
BitmapPtr ApplyAttributesToBitmap( BitmapPtr bitmap, ImageDimensions dimensions, ScalingMode scalingMode, SamplingMode samplingMode )
{
  ...
  if( bitmap )
  {
    ...
    bitmap = DownscaleBitmap( *bitmap, desiredDimensions, scalingMode, samplingMode );
    ...
    if( bitmap && bitmap->GetPackedPixelsProfile() && scalingMode == ScalingMode::ScaleToFill )
    {
                                        ...
                                        SamplingMode samplingMode )
{
      ...
      ((desiredWidth < bitmapWidth) || (desiredHeight < bitmapHeight)) )
  {
    ...
    DownscaleInPlacePow2( bitmap.GetBuffer(), pixelFormat, bitmapWidth, bitmapHeight, desiredWidth, desiredHeight, scalingMode, samplingMode, shrunkWidth, shrunkHeight );
