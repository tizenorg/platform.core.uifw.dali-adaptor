 ...
 */
template<typename PIXEL> inline void PointSampleAddressablePixels( const uint8_t * inPixels, unsigned int inputWidth,  unsigned int inputHeight, uint8_t * outPixels, unsigned int desiredWidth, unsigned int desiredHeight ) __attribute__((always_inline));
 ...
 */
template<typename PIXEL, PIXEL (*BilinearFilter) ( PIXEL tl, PIXEL tr, PIXEL bl, PIXEL br, unsigned int fractBlendHorizontal, unsigned int fractBlendVertical ), bool DEBUG_ASSERT_ALIGNMENT>
inline void LinearSampleGeneric( const unsigned char * __restrict__ inPixels, ImageDimensions inputDimensions, unsigned char * __restrict__ outPixels, ImageDimensions desiredDimensions ) __attribute__((always_inline));
template<
  typename PIXEL,
...
#if defined(DEBUG_ENABLED)
    std::cerr << "LinearSample():\n";
    std::cerr << "   in:  " << inDimensions << std::endl;
    std::cerr << "   out: " << outDimensions << std::endl;
#endif
  }
  else
  {
