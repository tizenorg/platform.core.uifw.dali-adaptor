    ...
    if( filteredWidth < shrunkWidth || filteredHeight < shrunkHeight )
    {
      if( filterMode == ImageAttributes::Linear || filterMode == ImageAttributes::BoxThenLinear )
      {
        ...
        if( outputBitmap )
        {
          ...
          {
            LinearSample1BPP( bitmap.GetBuffer(), shrunkWidth, shrunkHeight, outputBitmap->GetBuffer(), filteredWidth, filteredHeight );
          }
 ...
 * L8, A8
 */
#define PIXEL uint8_t ///< Temp, function will be generic.
                       ...
                       unsigned int desiredHeight )
{
  ...
  for( unsigned int outY = 0; outY < desiredHeight; ++outY )
  {
    ...
    for( unsigned int outX = 0; outX < desiredWidth; ++outX )
    {
      ...
      outScanline[outX] = BilinearFilter1BPP( pixel1, pixel3, pixel2, pixel4, inputXWeight, inputYWeight );
