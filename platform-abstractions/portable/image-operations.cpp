    ...
    unsigned int desiredWidth, unsigned int desiredHeight )
{
  ...
  for( unsigned int outY = 0; outY < desiredHeight; ++outY )
  {
    ...
    for( unsigned int outX = 0; outX < desiredWidth; ++outX )
    {
      ...
      // Per-pixel bounds check: (Can be removed once this code is trusted):
      DALI_ASSERT_DEBUG( integerX < inputWidth );
      DALI_ASSERT_DEBUG( reinterpret_cast<const uint8_t*>(inPixelAddress) < ( inPixels + inputWidth * inputHeight * sizeof(PIXEL) ) );
      DALI_ASSERT_DEBUG( reinterpret_cast<uint8_t*>(&outScanline[outX]) < ( outPixels + desiredWidth * desiredHeight * sizeof(PIXEL) ) );
