    ...
    unsigned& outWidth, unsigned& outHeight )
{
          ...
          scaledWidth >> 1u >= 1u           && scaledHeight >> 1u >= 1u )
  {
    ...
    DALI_LOG_INFO( gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleInPlacePow2RGB888() - Scaling to %u\t%u.\n", scaledWidth, scaledHeight );
    ...
    unsigned& outWidth, unsigned& outHeight )
{
          ...
          scaledWidth >> 1u >= 1u           && scaledHeight >> 1u >= 1u )
  {
    ...
    DALI_LOG_INFO( gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleInPlacePow2ComponentPair() - Scaling to %u\t%u.\n", scaledWidth, scaledHeight );
    ...
    unsigned& outWidth, unsigned& outHeight )
{
          ...
          scaledWidth >> 1u >= 1u           && scaledHeight >> 1u >= 1u )
  {
    ...
    DALI_LOG_INFO( gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleInPlacePow2RGBA8888() - Scaling to %u\t%u.\n", scaledWidth, scaledHeight );
...
/**
 * Apply rough, mip-map style power of 2 downscaling to a bitmap.
 * @note The input bitmap is modified and left in an invalid state so must be discarded.
 * @ToDo: Try to optimise by replacing the memcpy() with directly generating the final downscaling level in the destination bitmap.
 **/
Integration::BitmapPtr DownscaleBitmap( Integration::Bitmap& bitmap, const ImageAttributes& requestedAttributes, const std::string& path )
{
  ...
  DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 1 - checking if bitmap needs to be shrunk: pixelFormat: %u, dimensions(original: %u, %u, requested: %f, %f), \"%s\".\n", bitmap.GetPixelFormat(), bitmap.GetImageWidth(), bitmap.GetImageHeight(), requestedSize.x, requestedSize.y, path.c_str() );
  ...
  {
    DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 2 - bitmap does need to be shrunk: \"%s\".\n", path.c_str() );
    ...
    if( pixelFormat == Pixel::RGBA8888 || pixelFormat == Pixel::RGB888 || pixelFormat == Pixel::LA88 )
    {
      ...
      if( pixelFormat == Pixel::RGBA8888 )
      {
        ...
        {
          DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 3 - RGBA888 bitmap was shrunk.\n" );
      ...
      else if( pixelFormat == Pixel::RGB888 )
      {
        ...
        if( shrunkWidth != bitmapWidth && shrunkHeight != bitmapHeight )
        {
          DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 3 - RGB888 bitmap was shrunk.\n" );
      ...
      else if( pixelFormat == Pixel::LA88 )
      {
        ...
        if( shrunkWidth != bitmapWidth && shrunkHeight != bitmapHeight )
        {
          DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 3 - LA88 bitmap was shrunk.\n" );
      ...
      {
        DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 3 - bitmap was not shrunk (original: %u, %u, requested: %f, %f, shrunk: %u, %u.\n", bitmapWidth, bitmapHeight, requestedSize.x, requestedSize.y, shrunkWidth, shrunkHeight );
      }
    }
    else
    {
      DALI_LOG_INFO( Internal::Platform::gImageOpsLogFilter, Dali::Integration::Log::Verbose, "DownscaleBitmap() - 3 - bitmap was not shrunk: unsupported pixel format: %u.\n", unsigned(pixelFormat) );
    }
  }
 ...
 * @note This fakes the scaling with a crop and relies on the GPU scaling at render time. If the input bitmap was previously maximally downscaled using a repeated box filter, this is a reasonable approach.
 * @ToDo: Actually scale the image with a point or bilinear filter instead of just cropping to fake it.
 * @return The bitmap passed in if no scaling is needed or possible, else a new, smaller bitmap with the scaling mode applied.
 */
BitmapPtr ProcessBitmapScaleToFill( BitmapPtr bitmap, const ImageAttributes& requestedAttributes, const std::string& path, Integration::Log::Filter* logFilter = 0)
{
  ...
  else if( loadedWidth != desiredWidth || loadedHeight != desiredHeight )
  {
    ...
    DALI_LOG_INFO( logFilter, Debug::Concise, "ImageAttributes::ScaleToFill - Bitmap, desired(%f, %f), loaded(%u,%u), cut_target(%f, %f), trimmed(%u, %u), vertical = %s.\n", desiredDims.x, desiredDims.y, loadedWidth, loadedHeight, scaledDims.x, scaledDims.y, columnsToTrim, scanlinesToTrim, trimTopAndBottom ? "true" : "false" );

    // Make a new bitmap with the central part of the loaded one if required:
    if( scanlinesToTrim > 0 || columnsToTrim > 0 ) ///@ToDo: Make this test a bit fuzzy (allow say a 5% difference) if both dimensions are different from the raw ones.
    {
...
{
  DALI_LOG_INFO( gImageOpsLogFilter, Dali::Integration::Log::Verbose, " ⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽ > ApplyAttributesToBitmap() ⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽⱽ\n" );
  // If a different size than the raw one has been requested, resize the image
  ...
  DALI_LOG_INFO( gImageOpsLogFilter, Dali::Integration::Log::Verbose, " ^^^^^^^^^^^^^^^^^^^^ < ApplyAttributesToBitmap() ^^^^^^^^^^^^^^^^^^^^\n" );
  return bitmap;
}
