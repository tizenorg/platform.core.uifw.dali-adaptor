...
#include <integration-api/glyph-set.h>
#include <integration-api/platform-abstraction.h>
  ...
  const TextResourceType::CharacterList& requestedCharacters )
{
  ...
  for( std::size_t n = 0, arraySize = keyVector.size(); n < arraySize ; n++ )
  {
    ...
    if( data.exists )
    {
      ...
      BitmapPtr bitmapData ( Integration::Bitmap::New(Bitmap::BITMAP_2D_PACKED_PIXELS, ResourcePolicy::RETAIN) );
