...
// Android forward definition, (most likely it has to be in global namespace).
...
private:
  ...
  enum ImageType
  {
    eResourceFromAsset,        /// < create asset loader
    eResourceFromJava,         /// < images which we want to load from Java
    eInvalidImage
  };

  ResourceThreadImage::ImageType GetImageType(const std::string& path);
...
private:
  ...
  bool ConvertStreamToBitmap(const Integration::ResourceType& resourceType, const std::string& path, const Dali::Vector<uint8_t>& data, Integration::BitmapPtr& ptr);
