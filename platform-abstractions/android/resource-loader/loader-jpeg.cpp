...
bool LoadJpegHeaderInternalV(const std::vector<uint8_t>& jpegbuf, unsigned int &width, unsigned int &height, jpeg_decompress_struct &cinfo, jpeg_error_mgr &jerr)
{
	unsigned long jpegbufsize = jpegbuf.size();
	/* const */ unsigned char  *jpegbufPtr	= (unsigned char *)&jpegbuf[0];
	...
	jpeg_mem_src(&cinfo, jpegbufPtr, jpegbufsize);
...
bool LoadJpegMemLoaderHeaderV(jpeg_error_mgr &jerr)
{
...
bool LoadBitmapFromJpegVector(const std::vector<uint8_t>& jpegbuf, Bitmap& bitmap, ImageAttributes& attributes)
{
  ...
  unsigned long jpegbufsize = jpegbuf.size();
  unsigned char  *jpegbufPtr  = (unsigned char *)&jpegbuf[0];

  LoadJpegMemLoaderHeaderV( jerr);
  cinfo.err = jpeg_std_error(&jerr);
  ...
  ExifData *exifData;
  exifData = exif_data_new_from_data(jpegbufPtr, jpegbufsize);
  ExifEntry *entry;
  ExifTag tag = EXIF_TAG_ORIENTATION;
  entry = exif_data_get_entry(exifData, tag);
  int orientation = 0;
  if(entry)
  {
  ...
  // Load the header info to check cinfo has valid data @TODO nbdebug get rid of magic number
  unsigned int width, height;
  if (1 != jpeg_read_header(&cinfo, TRUE))
  {
      return false;
  }
  ...
  */
  ...
  // populate cinfo with image_width data etc.
  // jpeg_start_decompress(&cinfo);
  if(jpeg_start_decompress(&cinfo))
  	{
	  __android_log_print(ANDROID_LOG_DEBUG, " android LoadBitmapFromJpegV:", " start_decompress success %s %d\n", __PRETTY_FUNCTION__, __LINE__);
  	}
  else
  	{
	  __android_log_print(ANDROID_LOG_DEBUG, " android LoadBitmapFromJpegV:", " start_decompress failed %s %d\n", __PRETTY_FUNCTION__, __LINE__);
  	 	return false;
  	}
  ...
  else
  {
    ...
    case JPGFORM_ROT_90:
      pixels = bitmap.GetPackedPixelsProfile()->ReserveBuffer(pixelFormat, height,width, bufferHeight,bufferWidth);
    default:
      break;
    }
    lptr   = pixels;
...
bool LoadJpegHeaderV(const std::vector<uint8_t>& data, unsigned int &width, unsigned int &height)
{
	...
	return LoadJpegHeaderInternalV(data, width, height, cinfo, jerr);
}

bool LoadBitmapFromJpegV(const std::vector<uint8_t>& data, Bitmap& bitmap, ImageAttributes& attributes)
{
	return LoadBitmapFromJpegVector(data, bitmap, attributes);
}
