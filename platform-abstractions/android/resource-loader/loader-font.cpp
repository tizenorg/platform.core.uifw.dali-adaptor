                                  ...
                                  const bool renderBitmap, const bool highQuality)
{
  ...
  if (renderBitmap)
  {
    ...
    if(charcode == UNDERLINE_CHARACTER)
    {
      ...
      bitmapData = Integration::Bitmap::New(Bitmap::BITMAP_2D_PACKED_PIXELS, ResourcePolicy::RETAIN);
...
Integration::BitmapPtr GetGlyphBitmap( FT_Face face, FT_ULong charCode )
{
  ...
  if( NULL != ftGlyph )
  {
    ...
    if( 0 < size )
    {
      image = Integration::Bitmap::New( Integration::Bitmap::BITMAP_2D_PACKED_PIXELS, ResourcePolicy::RETAIN );
