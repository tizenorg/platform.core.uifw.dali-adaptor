...
#ifdef DALI_SUPPORTS_GIF_FORMAT
#include <gif_lib.h>
#endif
#include <cstdlib>
...
bool LoadGifHeader( const ImageLoader::Input& input, unsigned int& width, unsigned int& height )
{
  ...
  return LoadGifHeader(input.data, width, height, &gifInfo);
#else
  DALI_LOG_ERROR( "GIF Loader: Not Supported\n" );
  return false;
#endif
}
...
bool LoadBitmapFromGif( const ResourceLoadingClient& client, const ImageLoader::Input& input, Integration::Bitmap& bitmap )
{
  ...
  if ( !LoadGifHeader( input.data, width, height, &gifInfo ) )
  {
    return false;
  }
