...
#include <platform-abstractions/android/resource-loader/platform-capabilities.h>
...
bool LoadPngHeader( const ImageLoader::Input& input, unsigned int& width, unsigned int& height )
{
  ...
  reader.buffer = input.start;
  reader.bufsize = input.size;
...
bool LoadBitmapFromPng( const ResourceLoadingClient& client, const ImageLoader::Input& input, Integration::Bitmap& bitmap )
{
  ...
  bool crop = ( input.scalingParameters.scalingMode == FittingMode::SCALE_TO_FILL );
  ...
  if (crop)
  {
    Size req ((float) input.scalingParameters.dimensions.GetWidth(), (float) input.scalingParameters.dimensions.GetHeight());
    const Size orig ((float)width, (float)height);

    // calculate actual width, height
    req = FitScaleToFill(req, orig);
    unsigned int actualWidth = req.width;
    unsigned int actualHeight = req.height;

    bufferWidth  = GetTextureDimension(actualWidth);
    bufferHeight = GetTextureDimension(actualHeight);

    // cropped buffer's stride
    int lstride = bufferWidth*bpp;

    // calculate offsets
    int x_offset = ((width  - actualWidth)  / 2) * bpp;
    int y_offset = ((height - actualHeight) / 2) * stride;

    // allocate bitmap buffer using requested size
    unsigned char *bitmapBuffer = bitmap.GetPackedPixelsProfile()->ReserveBuffer(pixelFormat, actualWidth, actualHeight, bufferWidth, bufferHeight);
    ...
    {
      memcpy (bufptr, lptr, actualHeight * bpp);
      bufptr += lstride;
      lptr += stride;
    }
