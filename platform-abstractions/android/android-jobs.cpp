...
namespace Jobs
{
  ...
  void Job::DoExec(JNIEnv* env, void* payload)
  {
  ...
  JavaEnv::JavaEnv()
  {
    int jniAttached = mJvm->GetEnv((void**)&mEnv, JNI_VERSION_1_6);
    mDetached = (jniAttached == JNI_EDETACHED);
    if (mDetached)
    {
  ...
  struct Implementation
  {
    ...
    bool run_mainloop(JNIEnv* environment)
    {
      JobHandle job;
      while (MainQueue[MSGPRI_HIGH]->Pop(job))
      {
        job->DoExec(environment, NULL);
      }
    ...
    void WorkerFunction(uint32_t context)
    {
      ...
      while (!mShutdownRequested)
      {
        ...
        while (SharedQueue->Pop(job))
        {
          ...
          {
            job->DoExec(environment, (void*)context);
          }
          else
          {
            job->DoCancel();
  ...
  {
    return Implementation::mInstance->run_mainloop(environment);
  }
