...
struct PngFileReaderState
{
  ...
  PngFileReaderState(Dali::Platform::File* file)  : mFile(file)
  {
  }
...
bool LoadPngHeader(Dali::Platform::File* file_data, unsigned int &width, unsigned int &height, png_structp &png, png_infop &info)
{
...
bool LoadPngHeader(Dali::Platform::File* file_data, const ImageAttributes& attributes, unsigned int &width, unsigned int &height )
{
...
bool LoadBitmapFromPng( Dali::Platform::File* file_data, Bitmap& bitmap, ImageAttributes& attributes, const ResourceLoadingClient& client )
{
  png_structp png = NULL;
  png_infop info = NULL;
  auto_png autoPng(png, info);
