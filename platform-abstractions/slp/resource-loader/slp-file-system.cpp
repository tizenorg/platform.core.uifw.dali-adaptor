...
namespace Dali
{
  ...
  {
    class StandardFile : public Dali::Platform::File
    {
      FILE*       mFile;
      int64_t     mFileSize;
      std::string mName;
    ...
    class MemoryBackedFile : public Dali::Platform::File
    {
    protected:
    ...
    public:
      ...
      BufferedFile(const std::string& filename, Platform::FileSystemBase* filesystem) : mName(filename)
      {
        const size_t size = filesystem->GetFilesize(filename);
        if (size)
        {
          mData.Reserve(size);
          std::fstream inputStream(filename.c_str(), std::ios::in | std::ios::binary);
          if (inputStream.is_open())
          {
            inputStream.read(mData.Begin(), size);
            if (inputStream)
            {
...
Dali::Platform::File* Dali::SlpPlatform::FileSystem::OpenFile(const std::string& name)
{
  if (GetFilesize(name))
  {
...
Dali::Platform::File* Dali::SlpPlatform::FileSystem::MapToMemory(const uint8_t* data, size_t size)
{
