...
struct ResourceLoader::ResourceLoaderImpl
{
  ...
  Platform::FileSystemBase* mFilesystem;          ///< Abstracts away IO operations.
...
Platform::File* ResourceLoader::ReadFile(const std::string& filename)
{
  return mImpl->mFilesystem->OpenFile(filename);
}

Platform::File* ResourceLoader::MapToMemory(const uint8_t* bytes, const size_t length)
{
