...
void ResourceLoader::ChooseFontFamilyName( Integration::Text& text )
{
  ...
  std::vector<Integration::FontPerCharacter*> fontPerCharacters = text.mFontPerCharacters;
  text.mFontPerCharacters.clear();

  // Gets the first font per character struct.
  std::vector<Integration::FontPerCharacter*>::const_iterator fontPerCharacterIt = fontPerCharacters.begin();
  ...
  const std::size_t size = text.mText.size();
  for( std::size_t index = 0u, traversedCharacters = 0u; index < size; )
  {
    if( fontPerCharacter->mIsEmoji )
    {
      ...
      if( 0u < newFontPerCharacter->mNumberOfCharacters )
      {
        ...
        text.mFontPerCharacters.push_back( newFontPerCharacter );
    ...
    charsRequested.push_back( character );
  ...
  for( std::vector<Integration::FontPerCharacter*>::iterator it = fontPerCharacters.begin(), endIt = fontPerCharacters.end(); it != endIt; ++it )
  {
    delete *it;
  }
}
