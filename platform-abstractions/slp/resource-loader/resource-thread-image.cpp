...
typedef bool (*LoadBitmapFunction)( FILE*, Bitmap&, ImageAttributes&, const Dali::Internal::Platform::ImageLoaderClient& ); ///@ToDo: Make attributes a const reference?
...
void ResourceThreadImage::Load(const ResourceRequest& request)
{
  ...
  if( fp != NULL )
  {
    ...
    CheckForCancellation(); // Note: This can throw an exception.
    if( result && bitmap )
    {
 ...
 *
 * Used by clients such as image loaders via an interface class so
 * they don't have to know about the ResourceThreadImage type.
 **/
void ResourceThreadImage::InterruptForCancellation() const
{
