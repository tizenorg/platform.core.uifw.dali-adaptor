...
void ResourceThreadImage::Load(const ResourceRequest& request)
{
  ...
  Integration::FileDataHelper file_data = mResourceLoader.ReadFile(request.GetPath().c_str());
...
void ResourceThreadImage::Decode(const ResourceRequest& request)
{
  ...
  if( encodedBlob != 0 )
  {
    ...
    if( blobBytes != 0 && blobSize > 0U )
    {
      ...
      Integration::FileDataHelper file_data = mResourceLoader.MapToMemory(blobBytes, blobSize);
      if (file_data->IsOpen())
      {
