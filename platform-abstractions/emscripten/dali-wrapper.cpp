...
struct EmscriptenSignal
{
  ...
  bool operator()()
  {
    Dali::Actor a = Stage::GetCurrent().GetRootLayer().FindChildById(mId);
    if(a)
    {
...
void ActorWrapper::SetImageData(unsigned int width, unsigned int height, const std::string& data)
{
  ImageActor imageActor = ImageActor::DownCast(mActor);

  if(imageActor)
  {
    BitmapImage b = BitmapImage::New( width, height, Pixel::RGBA8888 ); // below expects 4 bytes

    const PixelBuffer* from = reinterpret_cast<const PixelBuffer*>( data.c_str() );
    PixelBuffer* to = b.GetBuffer();
...
void ActorWrapper::SignalConnected( SlotObserver* slotObserver, CallbackBase* callback )
{
  // pass
}

void ActorWrapper::SignalDisconnected( SlotObserver* slotObserver, CallbackBase* callback )
{
  // pass
}
...
CameraActorWrapper RenderTaskWrapper::GetCameraActor()
{
  return CameraActorWrapper( mRenderTask.GetCameraActor() );
}
...
//;
  ...
  function("sendMouseEvent", &EmscriptenMouseEvent);
  ...
  function("__createSolidColorActor", &CreateSolidColorActor);
...
;
  ...
  class_<ActorWrapper, base<HandleWrapper>>("ActorWrapper")
    ...
    .function("__setImageData", &ActorWrapper::SetImageData)
...
;
  ...
  class_<RenderTaskListWrapper>("RenderTaskListWrapper")
    .function("createTask", &RenderTaskListWrapper::CreateTask)
    .function("removeTask", &RenderTaskListWrapper::RemoveTask)
    .function("getTaskCount", &RenderTaskListWrapper::GetTaskCount)
    .function("getTask", &RenderTaskListWrapper::GetTask)
;

  class_<RenderTaskWrapper>("RenderTaskWrapper")
    .function("getCameraActor", &RenderTaskWrapper::GetCameraActor)
;
...
// };
  ...
  class_<CameraActorWrapper>("CameraActorWrapper")
    // .smart_ptr<std::shared_ptr<AnimationWrapper>>("AnimationWrapper")
