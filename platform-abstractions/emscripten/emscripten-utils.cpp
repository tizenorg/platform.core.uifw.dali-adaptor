...
Dali::BufferImage BufferImageNew(const unsigned int width, const unsigned int height, const Dali::Pixel::Format& format, const std::string& data)
{
  Dali::BufferImage b = Dali::BufferImage::New( width, height, format );

  const Dali::PixelBuffer* from = reinterpret_cast<const Dali::PixelBuffer*>( data.c_str() );
  Dali::PixelBuffer* to = b.GetBuffer();

  unsigned int len = std::max( width * height * 3, static_cast<unsigned int>(data.size()) );
