  ...
  for(unsigned int i = 0; i < valueLength; i++)
  {
    ...
    for(unsigned int j = 0; j < itemLength; ++j)
    {
      switch(type)
      {
        case Dali::Property::BOOLEAN:
        {
          *(reinterpret_cast<bool*>(data)) = item.as<bool>();
          data += sizeof(bool);
          break;
        }
        case Dali::Property::FLOAT:
        {
          *(reinterpret_cast<float*>(data)) = item.as<float>();
          data += sizeof(float);
          break;
        }
        case Dali::Property::INTEGER:
        {
          *(reinterpret_cast<int*>(data)) = item.as<int>();
          data += sizeof(int);
          break;
        }
        case Dali::Property::VECTOR2:
...
{
  Dali::PropertyBuffer buffer;

  Dali::Property::Value propertyValue;

  RecursiveSetProperty(propertyValue, val);

  DALI_ASSERT_ALWAYS(propertyValue.GetType() == Dali::Property::MAP);

  if( propertyValue.GetType() == Dali::Property::MAP )
  {
    Dali::Property::Map map = propertyValue.Get<Dali::Property::Map>();
    buffer = Dali::PropertyBuffer::New(map, size);
  }
