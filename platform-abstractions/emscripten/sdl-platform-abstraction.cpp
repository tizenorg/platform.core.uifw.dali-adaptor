...
//         if(!convertedSurface)
//         {
//           printf("  Could not convert image : %s\n", SDL_GetError());
//         }
...
//       else
//       {
//         printf("  Could not convert pixelimage : %s\n", SDL_GetError());
//       }
      ...
      if(surface)
      {
        ...
        unsigned char *pBytes = static_cast<unsigned char*>(surface->pixels);
        if( 0x89 == *(pBytes+0) && 0x50 == *(pBytes+1) ) // magic bytes for png_all_filters
        {
          pBytes +=8;                                    // 8 bytes for header
          uint32_t ihdr_len = *(pBytes+=4);              // 4 bytes for chunk length
          uint32_t ihdr_type= *(pBytes+=4);              // 4 bytes for chunk type
          // ihdr data (must be first chunk)
          uint32_t ihdr_width =  *(pBytes+=4);           // 4 for width,height
          uint32_t ihdr_height=  *(pBytes+=4);
          unsigned char ihdr_bitDepth=  *(pBytes+=1);    // 1 for bit depth
          unsigned char ihdr_colorType= *(pBytes+=1);    // 1 for bit colorType
        ...
        // printf("    index: %d memory: %d\n", index, (long long)(memory));
      } // if surface
      else
      {
        printf("  Error empty surface when decoding image? (SDL RW Memory ptr=%d) %s\n", (long long)(memory), SDL_GetError());
      }
...
void SdlPlatformAbstraction::GetTimeMicroseconds(unsigned int &seconds, unsigned int &microSeconds)
{
  ...
  // static double msNow = static_cast<long>(emscripten_get_now());

  seconds         = static_cast<unsigned int>(current/1000.0);
  microSeconds    = (static_cast<unsigned int>(current) - seconds*1000.0) * 1000;

  // int x = EM_ASM_INT({
  //     console.log('GetTimeMicroseconds: ' + [$0,$1,$2, $3]);
  //   }, current, msNow, (int)seconds, (int)microSeconds);
