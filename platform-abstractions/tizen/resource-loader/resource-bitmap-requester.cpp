...
{
  // @note if we ever have more than one thread using lib curl
  // we must explicitly call curl_global_init()
...
void ResourceBitmapRequester::LoadResource( Integration::ResourceRequest& request )
{
  ...
  else
  {
    ...
    {
      requestType = ResourceThreadBase::RequestHttpDownload;
      scheme = HTTP_RESOURCE;
    }
    else
    {
